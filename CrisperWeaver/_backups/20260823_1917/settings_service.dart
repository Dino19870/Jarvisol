...
class SettingsService {
  ...
  String get defaultMaleVoice => _prefs.getString('default_male_voice') ?? 'qwen3-ethan';
  ...
  Map<String, dynamic> getSpeakerVoiceConfig(String speakerId) {
    final raw = _prefs.getString('speaker_voice_config_$speakerId');
    if (raw == null || raw.isEmpty) return {};
    try {
      final decoded = jsonDecode(raw);
      if (decoded is Map<String, dynamic>) return decoded;
  ...
  List<VoicePreset> get customVoicePresets {
    final raw = _prefs.getString('custom_voice_presets');
    if (raw == null || raw.isEmpty) return VoicePreset.defaultPresets;
    try {
      final decoded = jsonDecode(raw);
      if (decoded is List) {
        final list = decoded.map((e) => VoicePreset.fromJson(e as Map<String, dynamic>)).toList();
  ...
  List<ClonedVoiceProfile> get customClonedVoices {
    final raw = _prefs.getString('custom_cloned_voices');
    if (raw == null || raw.isEmpty) return const [];
    try {
      final decoded = jsonDecode(raw);
      if (decoded is List) {
        return decoded
            .map((e) => ClonedVoiceProfile.fromJson(e as Map<String, dynamic>))
            .where((p) => p.wavPath.isNotEmpty && File(p.wavPath).existsSync())
            .toList();
      }
    } catch (_) {}
    return const [];
  ...
  Future<void> saveClonedVoice(ClonedVoiceProfile profile) async {
    ...
    await _prefs.setString(
        'custom_cloned_voices', jsonEncode(list.map((e) => e.toJson()).toList()));
  }
  ...
  List<CloudLlmProviderProfile> get customCloudProviders {
    final raw = _prefs.getString('custom_cloud_llm_providers');
    if (raw == null || raw.isEmpty) return [];
    try {
      final decoded = jsonDecode(raw) as List<dynamic>;
      return decoded
          .map((e) => CloudLlmProviderProfile.fromJson(e as Map<String, dynamic>))
          .toList();
    } catch (_) {
      return [];
    }
