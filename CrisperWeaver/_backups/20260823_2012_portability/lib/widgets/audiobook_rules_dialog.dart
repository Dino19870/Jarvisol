...
class _AudiobookRulesDialogState extends State<AudiobookRulesDialog> {
  ...
  void initState() {
    super.initState();
    _profiles = List.from(AudiobookRuleProfile.defaultProfiles);
...
Il n'agissait pas au hasard, ce n'était pas un « fou ». Wilshire était à gauche.''';
  ...
  Future<void> _showAddRuleDialog() async {
    ...
    if (result != null) {
      ...
      setState(() => _selectedProfile = _selectedProfile.copyWith(cleaningRules: updatedRules));
                ...
                child: DropdownButton<String>(
                  ...
                  onChanged: (val) {
                    if (val != null) {
                      ...
                      setState(() => _selectedProfile = found);
                      widget.onProfileSelected(found);
                      _runLivePreview();
                    }
                  },
                ),
