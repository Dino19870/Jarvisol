  ...
  Future<void> enableFileSink(bool enable) async {
    ...
    if (enable) {
      try {
        ...
        // Cross-platform public Documents/CrisperWeaver mirror (Android, Windows, macOS, Linux)
        if (!kIsWeb) {
          try {
            Directory? publicDir;
            if (Platform.isAndroid) {
              publicDir = Directory('/storage/emulated/0/Documents/CrisperWeaver');
            } else if (Platform.isWindows) {
              final userProfile = Platform.environment['USERPROFILE'];
              if (userProfile != null) {
                publicDir = Directory(p.join(userProfile, 'Documents', 'CrisperWeaver'));
              }
            } else if (Platform.isMacOS || Platform.isLinux) {
              final home = Platform.environment['HOME'];
              if (home != null) {
                publicDir = Directory(p.join(home, 'Documents', 'CrisperWeaver'));
              }
            }
            if (publicDir != null) {
