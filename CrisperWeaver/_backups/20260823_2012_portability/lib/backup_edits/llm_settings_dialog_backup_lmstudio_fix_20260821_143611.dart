    ...
    builder: (ctx) => StatefulBuilder(
      builder: (ctx, setDialogState) {
        Future<void> fetchAndroidModels() async {
          ...
          try {
            if (plat.isAndroid) {
              final List<dynamic>? rawList = await const MethodChannel('crisperweaver/litert_lm')
                  .invokeMethod<List<dynamic>>('listLocalModels');
              ...
              final regPaths = LiteRtModelRegistry().models
                  .where((m) => m.isDownloaded && m.localPath != null && m.localPath!.isNotEmpty)
                  .map((m) => m.localPath!)
                  .toList();
              final combined = <String>{...paths, ...regPaths};
              if (settings.llmModel.isNotEmpty) {
              ...
              final models = await llm.fetchModelsForEndpoint(urlController.text);
              await LiteRtModelRegistry().init();
              await LiteRtModelRegistry().refreshLocalStatus();
              final registryModels = LiteRtModelRegistry().models
                  .where((m) => m.isDownloaded && m.localPath != null && m.localPath!.isNotEmpty)
                  .map((m) => m.id)
                  .toList();
              final combined = <String>{...models, ...registryModels};
              if (settings.llmModel.isNotEmpty) {
                combined.add(settings.llmModel);
              }
              if (ctx.mounted) {
                setDialogState(() {
                  ...
                  isFetchingModels = false;
                  if (combined.isNotEmpty && (modelController.text.isEmpty || !combined.contains(modelController.text))) {
                    modelController.text = combined.contains('gemma-4-gpu') ? 'gemma-4-gpu' : combined.first;
                  }
                });
              }
            }
        ...
        Future<void> fetchModels() async {
          if (selectedProvider == LlmProvider.liteRtAndroid || selectedProvider == LlmProvider.liteRtWindows) {
        ...
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                const Text(
                  'Connectez CrisperWeaver à votre serveur local ou moteur IA embarqué.',
                ...
                DropdownButtonFormField<LlmProvider>(
                  ...
                  decoration: const InputDecoration(
                    labelText: 'Fournisseur LLM',
                    border: OutlineInputBorder(),
                    isDense: true,
                  ),
                  items: LlmProvider.values.where((p) {
                    if (plat.isAndroid) {
                      return p == LlmProvider.liteRtAndroid || p == LlmProvider.custom;
                    } else {
                      return p != LlmProvider.liteRtAndroid;
                    }
                  }).map((p) {
                  ...
                  onChanged: (p) {
                    if (p != null) {
                      setDialogState(() {
                        selectedProvider = p;
                        urlController.text = p.defaultEndpoint;
                        dialogModels.clear();
                      });
                      fetchModels();
                    }
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if (selectedProvider == LlmProvider.liteRtAndroid && settings.llmModel.isNotEmpty) {
                  try {
                    await const MethodChannel('crisperweaver/litert_lm')
      ...
      children: [
        ...
        Builder(
          builder: (bCtx) {
            ...
            final isContextTooHigh = userConfiguredContext > recCapacity;
