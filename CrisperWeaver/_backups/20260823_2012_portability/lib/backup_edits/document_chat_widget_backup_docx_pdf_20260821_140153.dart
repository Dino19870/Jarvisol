...
Résume, présente ou analyse directement ces courriels (expéditeurs, sujets, dates). Ne dis JAMAIS que tu n'as pas accès à la boîte mail puisque les données sont transmises ci-dessus.''';
  ...
  Future<void> _exportDiscussion(String format) async {
    try {
      String content = '';
      String ext = format;
      if (format == 'md') content = _generateMarkdownExport();
      else if (format == 'txt') content = _generatePlainTextExport();
      else if (format == 'html') content = _generateHtmlExport();
      else if (format == 'json') content = _generateJsonExport();

      final timeStamp = DateTime.now().toIso8601String().replaceAll(':', '-').split('.').first;
      final defaultFileName = 'Discussion_CrisperWeaver_$timeStamp.$ext';

      final encodedBytes = Uint8List.fromList(utf8.encode(content));
      String? savePath;
      if (plat.isDesktop) {
          ...
          child: Column(
            ...
            children: [
              ...
              _buildExportFormatOption(
                ...
                description: 'Recommandé : titres, blocs de code, listes et mise en page riche.',
              ...
              _buildExportFormatOption(
                ...
                description: 'Mise en page stylisée prête pour consultation et impression PDF.',
              ...
              _buildExportFormatOption(
                ...
                description: 'Format texte standard lisible sur n\'importe quel appareil ou éditeur.',
                    ...
                    itemBuilder: (context, index) {
                      if (index == _messages.length && _isStreaming) {
                        return _buildAssistantMessage(
                          _currentStreamingText.isEmpty ? 'Réflexion en cours...' : _currentStreamingText,
                          isStreaming: true,
                        );
                      }
  ...
  Widget _buildAssistantMessage(String text, {bool isStreaming = false}) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
