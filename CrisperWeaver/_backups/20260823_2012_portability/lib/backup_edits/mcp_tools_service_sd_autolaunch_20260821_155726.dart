...
class McpToolsService {
  ...
  /// Calls the local OpenAI-compatible Text-to-Image endpoint (/v1/images/generations).
  Future<String> generateImage(String visualPrompt, {String size = '512x512'}) async {
    ...
    final url = endpoint.endsWith('/images/generations')
        ? endpoint
        : (endpoint.endsWith('/v1') ? '$endpoint/images/generations' : '$endpoint/v1/images/generations');

    Log.instance.i('mcp_tools', 'Calling Image Gen endpoint: $url with prompt: "$visualPrompt"');
    ...
    try {
      ...
      return '⚠️ Le serveur Text-to-Image a renvoyé un code ${response.statusCode} : ${response.body}';
    } catch (e) {
      Log.instance.e('mcp_tools', 'Image generation error: $e');
      return '⚠️ **Serveur de Génération d\'Images Injoignable** ($url)\n\n'
          'Détail de l\'erreur : $e\n\n'
          '💡 *Assurez-vous que votre moteur local (SD-Turbo, Stable Diffusion WebUI / ComfyUI / LocalAI) est démarré sur ${_settings.imageGenApiUrl}.*';
    }
  }
}
