...
void main() {
  group('BatchQueueNotifier.reorderByGrouping', () {
    ...
    tearDown(() async {
      // Drain the serialised write chain BEFORE disposing — the chain
      // may have pending disk writes (persist + deleteJob) whose handles
      // must be released before we attempt to delete the temp dir.
      await queue.whenPersisted();
      queue.dispose();
      if (await tempDir.exists()) {
        // Windows: Defender / shell-extension DLLs may briefly hold a
        // newly-written file after Dart releases the handle. Retry up
        // to 5× with 100 ms backoff — consistent with the same pattern
        // in batch_queue_service_test.dart.
