...
void main() {
  ...
  group('cleanupBatch', () {
    ...
    test('total failure throws CloudLlmTotalFailureException with fallback texts (AUD-CLEAN-01 / TNR-086)', () async {
      final svc = CloudLlmCleanupService(client: MockClient((_) async {
        return http.Response('{"error":"rate_limit_exceeded"}', 429);
      }));
      addTearDown(svc.dispose);
      ...
      try {
        ...
        fail('Should have thrown CloudLlmTotalFailureException');
      } on CloudLlmTotalFailureException catch (e) {
        expect(e.fallbackTexts, ['segment 1', 'segment 2']);
        expect(e.message, contains('Échec total du nettoyage LLM'));
      }
    });
