...
import 'package:jarvisol/models/audiobook_rules.dart';
import 'package:jarvisol/services/audiobook_service.dart';
import 'package:jarvisol/services/tts_service.dart';
import 'package:jarvisol/widgets/audiobook_import_dialog.dart';
import 'package:jarvisol/widgets/audiobook_rules_dialog.dart';
...
- Je suis prête.''';
    ...
    test('Diarization with OCR profile identifies hyphen dialogues', () {
      ...
      final chap = AudiobookChapter(
        ...
        title: 'Chapitre 1',
        rawText:
            '''- Merci, docteur. Le témoin est à vous. Jake rassemble ses affaires.
...
Il regarda la salle avec calme.''',
      ...
      final lines = svc.castChapterLinesSync(
          chapter: chap, speakers: speakers, profile: profile);
  ...
  group('AUDIT REGLES LIVRE AUDIO : Widgets & Boîtes de Dialogue', () {
    testWidgets('AudiobookRulesDialog renders live preview and profiles',
        (tester) async {
          ...
          child: MaterialApp(
            home: Scaffold(
              body: AudiobookRulesDialog(
                ...
                profiles: AudiobookRuleProfile.defaultProfiles,
      ...
      expect(find.text('Règles de Découpage & Nettoyage des Livres'),
          findsOneWidget);
    ...
    testWidgets(
        'AudiobookImportDialog prompts for RAG indexing and profile selection',
        (tester) async {
      await tester.pumpWidget(
        ProviderScope(
          child: MaterialApp(
