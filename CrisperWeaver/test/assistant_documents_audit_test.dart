...
import 'package:jarvisol/models/ai_knowledge_record.dart';
import 'package:jarvisol/models/prompt_item.dart';
...
void main() {
  ...
  group('AUDIT MODULE 4 : Actions & Dialogues de l Assistant Documents', () {
    ...
    test('4.6 Connecteur MCP Gmail : Traduction semantique en syntaxe API', () {
      ...
      // Le connecteur ajoute newer_than:30d pour les requêtes "derniers/récents"
      // afin de limiter le volume de résultats côté API Gmail.
      expect(qGeneral, equals('in:inbox newer_than:30d'));

      final qResume = mcpService.extractGmailQuery('resume mes derniers messages gmail');
      expect(qResume, equals('in:inbox newer_than:30d'));
    });
