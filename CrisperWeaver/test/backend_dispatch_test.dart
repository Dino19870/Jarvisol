...
void main() {
  ...
  group('CrispASR backend dispatch', () {
    ...
    test('every engine backend is catalogued (or intentionally engine-only)',
      ...
      const engineOnly = {
        ...
        // cosyvoice3-tts-rl is an RL-checkpoint alias for cosyvoice3-tts:
        // both are declared in availableBackends() under the same
        // #ifdef CA_HAVE_COSYVOICE3 block (crispasr_c_api.cpp L4329) and
        // the dispatch arm immediately normalises s->backend to
        // "cosyvoice3-tts" (L3412). No distinct GGUF, no separate
        // ModelDefinition required — only the LLM checkpoint file differs.
        'cosyvoice3-tts-rl',
      };
  ...
  group('CrispASR 0.6 parity end-to-end (opt-in)', () {
    ...
    // S3Gen vocoder is a mandatory companion: the engine's T3 AR transformer
    // emits speech tokens that the S3Gen flow-matching vocoder converts to
    // PCM. Without it, synthesize() returns 0 samples (engine log:
    // "S3Gen not loaded. Call chatterbox_set_s3gen_path first.").
    // This mirrors the production path in synthesize_screen.dart L386 where
    // selectedCodec (= ModelKind.codec companion) is always passed to
    // TtsService.prepare(codecName:) → s.setCodecPath().
    ...
    test('chatterbox synthesises non-zero PCM', tags: ['slow'], () {
      ...
      addTearDown(s.close);
      // S3Gen vocoder is mandatory: set before synthesize, exactly as
      // TtsService.prepare() does in production (synthesize_screen.dart L386).
      s.setCodecPath(chatterboxS3gen!);
      ...
      expect(pcm.length, greaterThan(0));
        ...
        skip: !libAvailable
            ...
            : chatterboxModel == null
                ? 'set CRISPASR_TEST_CHATTERBOX_MODEL to a downloaded chatterbox-t3-*.gguf'
                : chatterboxS3gen == null
                    ? 'set CRISPASR_TEST_CHATTERBOX_S3GEN to a downloaded '
                        'chatterbox-s3gen-*.gguf (mandatory S3Gen vocoder companion)'
                    : null);
