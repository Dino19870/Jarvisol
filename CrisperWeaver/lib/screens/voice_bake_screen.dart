...
class _VoiceBakeScreenState extends ConsumerState<VoiceBakeScreen> {
  ...
  final _pythonController =
      TextEditingController(text: VoiceBakingService.defaultPythonExecutable);
  ...
  Future<void> _bake() async {
    ...
    try {
      ...
      final out = await svc.bake(
        ...
        // Forward the UI attestation explicitly — the service rejects the
        // request immediately if false, independently of the button state.
        voiceRightsAttested: _voiceRightsAttested,
        pythonExecutable: _pythonController.text.trim().isEmpty
            ? VoiceBakingService.defaultPythonExecutable
