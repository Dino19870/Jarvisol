...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
    ...
    return _buildSettingsSection(
      ...
      children: [
        ...
        ListTile(
          ...
          subtitle: const Text(
              'Génération et inpainting (Stable Diffusion, FLUX, Chroma)'),
          trailing: const Icon(Icons.chevron_right),
          onTap: () => context.push('/models/image'),
        ),
        ListTile(
        ...
        if (plat.isMacOS || plat.isWindows || plat.isLinux)
          ListTile(
            ...
            subtitle: Text(!settings.hotkeyEnabled ||
                ...
                : (ref.watch(hotkeyServiceProvider).lastResult.isConflict
                    ? '${settings.hotkeyCombo} ⚠️ (Conflit OS 1409)'
  ...
  Future<void> _showSpeakerVocabEditor() async {
    ...
    final speakersDir = '${AppPaths.dataDir.path}/speakers';
    // Ensure dir exists
      ...
      children: [
        if (Platform.isWindows)
          ListTile(
            ...
            title: const Text('Diagnostic Matériel & Capacités'),
            subtitle: const Text('Évaluer la compatibilité des moteurs IA avec cet ordinateur'),
            trailing: const Icon(Icons.chevron_right),
            onTap: () => HardwareDiagnosticDialog.show(context),
          ),
  ...
  Widget _buildAboutSection() {
    return _buildSettingsSection(
      ...
      children: [
        ...
        ListTile(
          title: Text(AppLocalizations.of(context).settingsAboutCrisperWeaver.replaceAll('CrisperWeaver', 'Jarvisol')),
          subtitle: Text(
