...
class SynthesizeScreen extends ConsumerStatefulWidget {
  ...
  /// Whether [modelName] requires a reference transcript when cloning
  /// from a WAV file.
  static bool requiresRefTextForModel(String? modelName) {
    if (modelName == null) return false;
    final m = modelName.toLowerCase();
    return (m.contains('qwen') && m.contains('base')) ||
        (m.contains('vibevoice') && (m.contains('1.5b') || m.contains('base')));
  }

  /// Evaluates the pre-flight guard for reference transcript on WAV cloning.
  static bool shouldBlockSynthesisForMissingRefText({
    required bool hasCustomWav,
    required String? modelName,
    required String? refText,
  }) {
    if (!hasCustomWav) return false;
    if (!requiresRefTextForModel(modelName)) return false;
    return refText == null || refText.trim().isEmpty;
  }
...
class _SynthesizeScreenState extends ConsumerState<SynthesizeScreen> {
  ...
  void initState() {
    ...
    final initialModel = widget.initialModelName;
    if (initialModel != null && initialModel.isNotEmpty) {
      ref.read(synthesizeScreenProvider.notifier).setSelectedModel(initialModel);
    }
  ...
  Future<void> _refresh() async {
    ...
    try {
      ...
      // Load imported voice packs from portable storage
      final importedVoices =
          await ref.read(importedVoiceServiceProvider.notifier).loadAll();
      n.setImportedVoices(importedVoices);
  ...
  Future<void> _onVoicePicked(
    String? name,
    List<ModelInfo> voices,
    List<ImportedVoicePack> imported,
  ) async {
    if (name == null) {
      ref.read(synthesizeScreenProvider.notifier).setSelectedVoice(null);
      return;
    }
    final sn = ref.read(synthesizeScreenProvider.notifier);
    sn.setSelectedVoice(name);
    sn.setCustomVoiceWavPath(null);

    final matchImported =
        imported.where((v) => v.fileName == name || v.id == name).toList();
    if (matchImported.isNotEmpty) {
      final pack = matchImported.first;
      if (pack.presetSpeakers.isNotEmpty) {
        sn.setPresetSpeakers(pack.presetSpeakers);
        sn.setSelectedSpeaker(pack.presetSpeakers.first);
      } else {
        sn.resetSpeakerState();
      }
      return;
    }

    final matchCatalog = voices.where((m) => m.name == name).toList();
    if (matchCatalog.isNotEmpty) {
      final picked = matchCatalog.first;
      if (!picked.isDownloaded) {
        await _downloadCompanion(picked);
      }
    }
  }

  Future<void> _importVoicePack() async {
    try {
      final result = await pickFilesRobust(
        type: FileType.custom,
        allowedExtensions: ['gguf'],
      );
      if (result.localPaths.isEmpty) return;
      final sourcePath = result.localPaths.first;

      // 1. Validation stricte du GGUF
      ...
      if (!info.isValid) {
        ...
        await showDialog<void>(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: Text(
              'Le fichier sélectionné n\'est pas un Voice Pack GGUF valide.\n\n'
              '${info.errorMessage ?? "Seuls les formats Chatterbox (arch: chatterbox-voice) et Qwen3-TTS (arch: qwen3tts.voicepack) contenant les tenseurs de conditionnement requis sont acceptés."}',
            ),
            actions: [
              FilledButton(
      ...
      // 2. Dialogue de confirmation préalable
      final fileName = p.basename(sourcePath);
      final isChatterbox = info.architecture == 'chatterbox-voice';
      final typeDesc = isChatterbox ? 'Chatterbox Voice' : 'Qwen3-TTS Voice Pack';
      final compatibleDesc = isChatterbox ? 'Chatterbox TTS' : 'Qwen3-TTS Base';
      final voicesDesc =
          info.voiceNames.isEmpty ? '(inconnu)' : info.voiceNames.join(', ');

      final targetFile =
          File(p.join(AppPaths.importedVoicesDir.path, fileName));
      final alreadyExists = targetFile.existsSync();

      final confirm = await showDialog<bool>(
        context: context,
        builder: (ctx) => AlertDialog(
          title: const Text('Confirmer l\'import du Voice Pack'),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            ...
            children: [
              ...
              Text('Voix incluse(s) : $voicesDesc'),
              const SizedBox(height: 6),
              Text('Compatible avec : $compatibleDesc'),
              const SizedBox(height: 6),
              Text('Fichier source : $fileName'),
              if (alreadyExists) ...[
                ...
                Container(
                  ...
                  child: Text(
                    'Attention : Un Voice Pack avec ce nom de fichier existe déjà dans le stockage. '
                    'Confirmer l\'import remplacera la version existante.',
                    style: TextStyle(
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          '${info.warningMessage!}\nSi la transcription contient du texte non prononcé dans l\'audio d\'origine, le modèle le vocalisera à la synthèse.',
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () => Navigator.of(ctx).pop(true),
              child: Text(alreadyExists ? 'Remplacer' : 'Importer'),
            ),
          ],
        ),
      );
      ...
      // 3. Copie sécurisée dans le stockage portable
      final voiceService = ref.read(importedVoiceServiceProvider.notifier);
      final imported = await voiceService.importVoicePack(
        sourcePath,
        overwrite: alreadyExists,
      );

      // 4. Mettre à jour l'état local
      final allImported = voiceService.allPacks;
      final sn = ref.read(synthesizeScreenProvider.notifier);
      sn.setImportedVoices(allImported);

      // 5. Sélection immédiate du Voice Pack importé
      sn.setSelectedVoice(imported.fileName);
      sn.setCustomVoiceWavPath(null);
      if (imported.presetSpeakers.isNotEmpty) {
        sn.setPresetSpeakers(imported.presetSpeakers);
        sn.setSelectedSpeaker(imported.presetSpeakers.first);
      } else {
        sn.resetSpeakerState();
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              'Voice Pack "${imported.displayName}" importé et sélectionné avec succès.'),
        ),
      );
    } catch (e, st) {
      Log.instance.e('synth', 'Voice pack import error', error: e, stack: st);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
  ...
  Future<void> _deleteSelectedImportedVoice(ImportedVoicePack pack) async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Supprimer la voix importée'),
        content: Text(
          'Voulez-vous vraiment supprimer la voix importée "${pack.displayName}" (${pack.fileName}) du stockage local ?\n\n'
    ...
    final service = ref.read(importedVoiceServiceProvider.notifier);
    await service.deleteVoicePack(pack.fileName);

    final allImported = service.allPacks;
    final sn = ref.read(synthesizeScreenProvider.notifier);
    sn.setImportedVoices(allImported);
    sn.setSelectedVoice(null);
    sn.resetSpeakerState();

    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('La voix "${pack.displayName}" a été supprimée.'),
      ),
    );
  }
  ...
  void _autoSelectCompanions() {
    ...
    final voices = s.allModels.where((m) =>
        ...
        m.isDownloaded).toList();
    final compatibleImported = s.importedVoices.where((v) =>
        isCompatible(v.compatibleBackend, modelDef.backend)).toList();
    final codecs = s.allModels.where((m) =>
        m.kind == ModelKind.codec &&
        isCompatible(m.backend, modelDef.backend) &&
        m.isDownloaded).toList();

    // Preserve current selection if it remains compatible
    final curVoice = s.selectedVoice;
    final curVoiceOk = curVoice != null && (
      voices.any((m) => m.name == curVoice) ||
      compatibleImported.any((v) => v.fileName == curVoice || v.id == curVoice)
    );

    if (!curVoiceOk) {
      if (voices.isNotEmpty) {
        n.setSelectedVoice(voices.first.name);
      } else if (compatibleImported.isNotEmpty) {
        final firstPack = compatibleImported.first;
        n.setSelectedVoice(firstPack.fileName);
        if (firstPack.presetSpeakers.isNotEmpty) {
          n.setPresetSpeakers(firstPack.presetSpeakers);
          n.setSelectedSpeaker(firstPack.presetSpeakers.first);
        } else {
          n.resetSpeakerState();
        }
      } else {
  ...
  Future<void> _synthesize() async {
    ...
    final hasCustomWav =
        ss.customVoiceWavPath != null && ss.customVoiceWavPath!.isNotEmpty;
    final refText = _refTextController.text.trim();

    if (SynthesizeScreen.shouldBlockSynthesisForMissingRefText(
      hasCustomWav: hasCustomWav,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(l.synthRefTextRequired),
    ...
    try {
      ...
      final isSpeakerCapable = modelDef?.backend != null &&
          _speakerCapableBackends.contains(modelDef!.backend);
      final isSpeakerIdCapable = modelDef?.backend != null &&
          _speakerIdCapableBackends.contains(modelDef!.backend);

      final effectiveSpeakerName =
          hasCustomWav ? null : (isSpeakerCapable ? ss.selectedSpeaker : null);
      final effectiveSpeakerId =
          hasCustomWav ? null : (isSpeakerIdCapable ? ss.selectedSpeakerId : null);
      final effectiveVoiceName = hasCustomWav ? null : ss.selectedVoice;

      Log.instance.i('synth', 'prepare', fields: {
        'model': ss.selectedModel ?? '',
        'voice': effectiveVoiceName ?? '',
        'codec': ss.selectedCodec ?? '',
        'speaker': effectiveSpeakerName ?? '',
        'customWav': hasCustomWav,
      });
      final status = await tts.prepare(
        modelName: ss.selectedModel!,
  ...
  Widget build(BuildContext context) {
    ...
    final hasCustomWav =
        ss.customVoiceWavPath != null && ss.customVoiceWavPath!.isNotEmpty;
    final importedVoices = ref.watch(importedVoiceServiceProvider);
    final ttsModels =
    ...
    final compatibleImported = importedVoices
        .where((v) => isCompatible(v.compatibleBackend, modelDef?.backend))
        .toList(growable: false);
    final codecs = ss.allModels
        .where(
            (m) => m.kind == ModelKind.codec && isCompatible(m.backend, modelDef?.backend))
        .toList(growable: false);
    final bool supportsVoicePacks =
        modelDef?.backend == 'chatterbox' || modelDef?.backend == 'qwen3-tts';
              ...
              child: Column(
                ...
                children: [
                  if (downloadedTtsModels.isEmpty)
                    ...
                    if (voices.isNotEmpty ||
                        compatibleImported.isNotEmpty ||
                        supportsVoicePacks) ...[
                      const SizedBox(height: 8),
                      if (voices.isNotEmpty || compatibleImported.isNotEmpty)
                        DropdownButtonFormField<String>(
                          decoration:
                              InputDecoration(labelText: l.synthVoiceLabel),
                          initialValue: (voices.any((m) => m.name == ss.selectedVoice) ||
                                  compatibleImported.any((v) =>
                                      v.fileName == ss.selectedVoice ||
                                      v.id == ss.selectedVoice))
                              ? ss.selectedVoice
                              : null,
                          items: [
                            ...voices.map((m) => DropdownMenuItem(
                                  value: m.name,
                                  child: Text(
                                    '${m.displayName}'
                            ...
                            ...compatibleImported.map((v) => DropdownMenuItem(
                                  value: v.fileName,
                                  child: Text(
                                    '${v.displayName} (Importé)',
                                    overflow: TextOverflow.ellipsis,
                                    style: TextStyle(
                          ...
                          onChanged: (v) =>
                              _onVoicePicked(v, voices, compatibleImported),
                        ),
                      const SizedBox(height: 4),
                      Builder(builder: (context) {
                        final selectedImported = compatibleImported
                            .where((v) =>
                                v.fileName == ss.selectedVoice ||
                                v.id == ss.selectedVoice)
                            .firstOrNull;
                        return Wrap(
                          ...
                          children: [
                            OutlinedButton.icon(
                              ...
                              label: const Text('Importer un Voice Pack (.gguf)'),
                              onPressed: _importVoicePack,
                            ),
                            if (selectedImported != null)
                              TextButton.icon(
                                ...
                                label: Text('Supprimer la voix importée',
                                    style: TextStyle(
                                        color: Theme.of(context)
                                            .colorScheme
                                            .error)),
                                onPressed: () =>
                                    _deleteSelectedImportedVoice(selectedImported),
                              ),
                          ],
                        );
                      }),
                    ...
                    if (ss.presetSpeakers.isNotEmpty) ...[
                      ...
                      if (hasCustomWav) ...[
                        InputDecorator(
                          decoration: InputDecoration(
                            ...
                            helperText:
                                'Clonage WAV actif — le speaker prédéfini est ignoré.',
                            border: const OutlineInputBorder(),
                            isDense: true,
                          ),
                          child: Text(
                            ss.selectedSpeaker != null
                                ? '${ss.selectedSpeaker} (inactif — WAV prioritaire)'
                                : 'Ignoré (WAV prioritaire)',
                            style: TextStyle(
                      ...
                      if (hasCustomWav) ...[
                        InputDecorator(
                          ...
                          child: Text(
                            'Speaker ${ss.selectedSpeakerId ?? 0} (inactif — WAV prioritaire)',
                            style: TextStyle(
                                color: Theme.of(context).disabledColor),
                          ),
                        ),
                        ...
                        DropdownButtonFormField<int>(
                          decoration: InputDecoration(
                            labelText: l.synthSpeakerLabel,
                            helperText:
                                '${ss.nSpeakers} speakers available (0-indexed)',
                          ),
