...
class _VoiceCloneWizardScreenState
  ...
  /// query parameters.
  Future<void> _finishToSynthesize() async {
    final path = _recordedPath;
    if (path == null) return;

    // Direct WAV cloning was fixed upstream to use Qwen3-TTS Base.  Do not
    // hand the WAV to whichever TTS model happened to be selected previously
    // (CustomVoice rejects arbitrary reference WAVs).
    ...
    String? modelName;
    try {
      final models = await ModelService(ref.read(settingsServiceProvider)).getWhisperCppModels();
      for (final id in preferred) {
        if (models.any((m) =>
            m.name == id && m.kind == ModelKind.tts && m.isDownloaded)) {
          modelName = id;
          break;
        }
      }
    } catch (_) {
      // The synth screen will surface a missing-model condition below.
    }
    if (!mounted) return;
    if (modelName == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Le clonage direct nécessite Qwen3-TTS Base. '
            'Téléchargez un modèle Base dans Modèles avant de continuer.',
          ),
