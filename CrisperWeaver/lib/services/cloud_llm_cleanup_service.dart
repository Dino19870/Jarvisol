...
class CloudLlmTotalFailureException implements Exception {
  final String message;
  final List<String> fallbackTexts;

  const CloudLlmTotalFailureException(this.message, {required this.fallbackTexts});

  @override
  String toString() => 'CloudLlmTotalFailureException: $message';
}
...
class CloudLlmCleanupService {
  ...
  /// If ALL segments fail, throws CloudLlmTotalFailureException (AUD-CLEAN-01 / TNR-086).
  Future<List<String>> cleanupBatch({
    ...
    // AUD-CLEAN-01 / TNR-086: Si tous les segments échouent (panne totale du provider LLM),
    // ne pas présenter le résultat comme un nettoyage réussi. Lever une exception explicite
    // tout en conservant les textes bruts de secours.
    if (texts.isNotEmpty && failureCount == texts.length && successCount == 0) {
      throw CloudLlmTotalFailureException(
        'Échec total du nettoyage LLM : tous les segments ont échoué.',
        fallbackTexts: out,
      );
    }
