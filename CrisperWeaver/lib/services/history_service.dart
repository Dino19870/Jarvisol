...
class HistoryEntry {
  ...
  /// User-edited continuous transcript.  Segment timestamps/diarization stay
  /// untouched when Full Text is edited, so this override is persisted
  /// separately instead of trying to redistribute arbitrary prose back over
  /// timestamped segments.
  final String? fullTextOverride;
  ...
  String get fullText {
    final edited = fullTextOverride?.trim();
  ...
  Map<String, dynamic> toJson() => {
        ...
        if (fullTextOverride != null && fullTextOverride!.trim().isNotEmpty)
          'fullTextOverride': fullTextOverride,
  ...
  static HistoryEntry fromJson(Map<String, dynamic> j) => HistoryEntry(
        ...
        fullTextOverride: j['fullTextOverride'] as String?,
...
class HistoryService {
  ...
  Future<void> update(HistoryEntry entry) async {
    ...
    // Callers that edit text/speaker labels intentionally send only the
    // mutable fields.  Preserve the immutable provenance from the original
    // record instead of replacing it with DateTime.now(), "crispasr", null
    // model/language/source, etc.  This also makes the implementation match
    // the contract documented above this method.
    final existing = await loadEntry(entry.id);
    ...
    final merged = HistoryEntry(
      ...
      fullTextOverride: entry.fullTextOverride ?? existing.fullTextOverride,
      sourcePath: existing.sourcePath,
      sourceUrl: existing.sourceUrl,
      modelId: existing.modelId,
      language: existing.language,
      diarizationEnabled: existing.diarizationEnabled,
      processingTime: existing.processingTime,
      speakerNames: entry.speakerNames,
      audioFingerprint: existing.audioFingerprint,
      // Text edits can invalidate segment embeddings.  Keep them only when
      // the segment text payload is unchanged; otherwise force lazy rebuild.
      segmentEmbeddings: _sameSegmentTexts(existing.segments, entry.segments)
          ? existing.segmentEmbeddings
          : null,
      audioEmbedding: existing.audioEmbedding,
    );
  ...
  Future<HistoryEntry> save({
    ...
    final file = File(p.join(dir.path, '${entry.id}.json'));
    final tmpFile = File(p.join(dir.path, '${entry.id}.json.tmp'));
      ...
      if (ent is File && ent.path.endsWith('.json')) {
        try {
          ...
          try {
            ...
            final corruptPath = '${ent.path}.bak_corrupt_$ts';
            await ent.rename(corruptPath);
            Log.instance.w('history', 'Quarantined corrupt history file to $corruptPath');
          } catch (qErr) {
            Log.instance.e('history', 'Failed to quarantine corrupt history file: $qErr');
          }
        }
      }
    }
