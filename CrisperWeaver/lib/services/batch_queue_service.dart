...
import 'package:flutter_riverpod/legacy.dart'; // Â§legacy: BatchQueueNotifier
// still extends StateNotifier because 18+ tests construct it directly
// (BatchQueueNotifier(persistence: â€¦)) without a ProviderContainer.
...
/// without re-reading global settings (Â§5.23 Q1 grouping) and so a
/// resumed-from-crash job knows which model to load.
///
/// `resumeOffsetSec` is the float second offset into the audio at which
/// to restart transcription after a crash. Populated by the Â§5.23 Q3
...
/// owns the runner â€” it drives the queue by calling `nextQueued()` in a
/// loop after each job finishes.
///
/// **Persistence (Â§5.23 Q1).** Every state mutation is mirrored to a
...
class BatchQueueNotifier extends StateNotifier<List<BatchJob>> {
  BatchQueueNotifier({
    ...
    /// Optional duration probe â€” called async after enqueue to stamp
    /// `durationSec` on the job for ETA estimation (Â§5.23 Q1).
  ...
  // Persistence and load health tracking (TNR-075)
  final Set<String> _unpersistedJobIds = {};
  ...
  Set<String> get unpersistedJobIds => Set<String>.unmodifiable(_unpersistedJobIds);
  bool get hasLoadError => _hasLoadError;
  String? get loadError => _loadError;

  /// Â§5.25.11 â€” Cached set of audio fingerprints from history for
  ...
  // Monotonically-incrementing suffix for job IDs. Appended to the
  // microsecond timestamp so multiple jobs enqueued within the same
  // microsecond (common in unit tests) still get distinct IDs and
  // distinct on-disk files (job-<ts>_<seq>.json).
  static int _idSeq = 0;
  ...
  // â”€â”€ Persistence serialization â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // Writes are fired fire-and-forget from UI mutations (never block the
  // frame); but tests need to drain ALL of them deterministically before
  // asserting on disk state. We chain every write onto a single tail future
  // (_writeChain) so that:
  //   â€¢ writes are serialised (no concurrent disk races across jobs),
  //   â€¢ whenPersisted() = _writeChain always represents the completion of
  //     *every* write scheduled so far, not just the last one in flight.
  //
  // Cost: writes are sequential rather than parallel. Acceptable because
  // (a) the batch queue is typically short, (b) each write is O(1 file),
  // (c) production never awaits whenPersisted() â€” it's test-only.
  Future<void> _writeChain = Future<void>.value();

  /// Schedule [writeFn] to run after all previously scheduled writes.
  ///
  /// [writeFn] is a factory (not a pre-started Future) so that the
  /// underlying disk I/O only starts after preceding writes finish â€”
  /// preventing concurrent writes that could race on the same file.
  void _fireAndForget(Future<void> Function() writeFn) {
    _writeChain = _writeChain
        .then((_) => writeFn())
        .catchError((Object e, StackTrace st) {
      // Persistence failures are non-fatal: the in-memory queue stays
      // consistent. Log and move on so the chain doesn't stall.
      Log.instance
          .w('batch-queue', 'persist error (swallowed): $e', error: e, stack: st);
    });
  }

  /// Awaits all in-flight persistence writes (save / delete / clear AND
  /// the post-enqueue duration probe). Test-only â€” production never blocks
  /// on disk. Since _writeChain is a serial tail, awaiting it guarantees
  /// every write scheduled before this call has completed on disk.
  @visibleForTesting
  Future<void> whenPersisted() => _writeChain;
  ...
  /// Hydrate in-memory state from the on-disk queue. Idempotent â€”
  /// safe to call multiple times. Used by the app-startup wiring in
  /// `main.dart`.
  ///
  /// Two repair passes (Â§5.23 Q1 + Q3):
  ...
  Future<void> load() async {
    ...
    try {
      ...
      _hasLoadError = false;
      _loadError = null;
      // Pass 1: demote running â†’ queued.
      ...
      // Push only the mutated jobs back to disk â€” saves I/O on the
      ...
      // Do NOT set _loaded = true on error, allowing retry in the same session.
      // Do NOT replace existing state with empty queue on transient I/O failure (TNR-075).
      _hasLoadError = true;
      _loadError = e.toString();
      Log.instance.w('batch-queue', 'load failed; state preserved, retry allowed: $e',
          error: e, stack: st);
    }
  }
  ...
  Future<void> _persist(BatchJob job) async {
    try {
      ...
      _unpersistedJobIds.remove(job.id);
      if (_unpersistedJobIds.isEmpty) {
        _hasPersistenceError = false;
        _lastPersistenceError = null;
      }
    } catch (e, st) {
      _unpersistedJobIds.add(job.id);
      _hasPersistenceError = true;
      _lastPersistenceError = e.toString();
      Log.instance.w('batch-queue', 'saveJob failed for ${job.id} (unpersisted): $e',
          error: e, stack: st);
    }
  }

  /// Re-attempt saving all unpersisted jobs to disk (TNR-075).
  Future<bool> retryPersistence() async {
    ...
    final toRetry = Set<String>.from(_unpersistedJobIds);
    bool allSuccess = true;
    for (final id in toRetry) {
      final job = state.cast<BatchJob?>().firstWhere(
            (j) => j?.id == id,
            orElse: () => null,
          );
      if (job != null) {
    ...
    _hasPersistenceError = _unpersistedJobIds.isNotEmpty;
    return allSuccess;
  }

  /// Â§5.25.11 â€” Async dedup check: computes the file fingerprint and
  ...
  String enqueue(String filePath,
    ...
    // Dedup by path â€” already-queued or in-flight jobs stay in place.
    ...
    // Â§5.23 Q1 ETA probe â€” fire-and-forget. The job is queued
    ...
    if (_durationProbe != null) {
      // Tracked through the chain so `whenPersisted()` drains the
      // probe â†’ stamp â†’ persist chain too â€” otherwise a test that asserts
      // on the stamped durationSec (in memory or on disk) races the
      // fire-and-forget probe. Harmless in prod: the chain is awaited
      // by nobody in production.
  ...
  Future<void> _probeAndStamp(String jobId, String filePath) async {
    try {
      ...
      // Intentionally caught â€” probe failure means "we don't know
  ...
  /// Done / error / cancelled / running jobs stay in place â€” only
  ...
  /// `settings.groupBatchByBackend` is true. Cheap â€” O(n log n).
  /// Â§5.23 Q1 grouping sub-bullet.
  ...
  void setDone(String id, {String? resultText, String? historyEntryId}) {
    ...
    // Clean up any leftover checkpoint file â€” the job finished cleanly
  ...
  /// Expose the persistence service for the Â§5.23 Q3 resume flow
...
final batchQueueProvider =
  ...
  return BatchQueueNotifier(
    // Wire the Â§5.23 Q1 duration probe â€” fires async after enqueue,
