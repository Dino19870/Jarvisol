...
class TranscriptionWorkerPool {
  ...
  static Future<TranscriptionWorkerPool> spawn({
    ...
    void Function(TranscriptionWorkerArgs)? workerEntryPoint,
  }) async {
    if (count < 1) {
  ...
  static Future<_Worker?> _spawnOne({
    ...
    try {
      isolate = await Isolate.spawn<TranscriptionWorkerArgs>(
        workerEntryPoint ?? transcriptionWorkerEntry,
    ...
    final effectiveTimeout = initTimeout ?? TimeoutPolicy.asrWorkerInitTimeout;
    final timeout = Timer(effectiveTimeout, () {
