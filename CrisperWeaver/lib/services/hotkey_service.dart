...
class HotkeyRegistrationResult {
  ...
  bool get isConflict => status == HotkeyRegistrationStatus.conflictAlreadyRegistered;
}
...
class HotkeyService {
  ...
  HotKey? _registered;
  HotkeyRegistrationResult _lastResult = const HotkeyRegistrationResult(status: HotkeyRegistrationStatus.disabled);

  HotkeyRegistrationResult get lastResult => _lastResult;
  ...
  Future<HotkeyRegistrationResult> applyFromSettings() async {
    if (!isPlatformSupported) {
      _lastResult = const HotkeyRegistrationResult(status: HotkeyRegistrationStatus.unsupported);
      return _lastResult;
    }
    await unregister();
    if (!_settings.hotkeyEnabled) {
      _lastResult = const HotkeyRegistrationResult(status: HotkeyRegistrationStatus.disabled);
      return _lastResult;
    }
    ...
    if (hotKey == null) {
      Log.instance.w('hotkey',
          ...
          fields: {'combo': combo});
      _lastResult = HotkeyRegistrationResult(
        status: HotkeyRegistrationStatus.otherOsError,
        errorMessage: 'Invalid combo format: $combo',
      );
      return _lastResult;
    }
    try {
      ...
      int? code;
      if (pe.details is Map && (pe.details as Map)['errorCode'] != null) {
        code = ((pe.details as Map)['errorCode'] as num).toInt();
      } else if (pe.code == '1409' || pe.code == 'HOTKEY_ALREADY_REGISTERED') {
        code = 1409;
      }
      final isConflict = code == 1409 || pe.code == 'HOTKEY_ALREADY_REGISTERED';
      Log.instance.w('hotkey', 'startup registration conflict',
          fields: {'combo': combo, 'code': code, 'message': pe.message});
      _lastResult = HotkeyRegistrationResult(
        status: isConflict
            ? HotkeyRegistrationStatus.conflictAlreadyRegistered
            : HotkeyRegistrationStatus.otherOsError,
        errorMessage: pe.message ?? pe.code,
        win32ErrorCode: code,
      );
      return _lastResult;
    } catch (e, st) {
      Log.instance.e('hotkey', 'startup register failed',
  ...
  Future<HotkeyRegistrationResult> registerCandidate({
    required bool enabled,
    required String combo,
    required HotkeyAction action,
  }) async {
    if (!isPlatformSupported) {
      return const HotkeyRegistrationResult(status: HotkeyRegistrationStatus.unsupported);
    }
    if (!enabled || combo.trim().isEmpty) {
      await unregister();
      _settings.hotkeyEnabled = enabled;
      _settings.hotkeyCombo = combo.trim();
      _settings.hotkeyAction = action;
      _lastResult = const HotkeyRegistrationResult(status: HotkeyRegistrationStatus.disabled);
      return _lastResult;
    }

    final newHotKey = parse(combo);
    if (newHotKey == null) {
      return HotkeyRegistrationResult(
        status: HotkeyRegistrationStatus.otherOsError,
        errorMessage: 'Invalid hotkey combo: $combo',
      );
    }

    final prevRegistered = _registered;
    final prevCombo = _settings.hotkeyCombo;
    final prevEnabled = _settings.hotkeyEnabled;
    ...
    try {
      ...
      _registered = newHotKey;
      _settings.hotkeyEnabled = enabled;
      _settings.hotkeyCombo = combo.trim();
      _settings.hotkeyAction = action;
      Log.instance.i('hotkey', 'registered candidate successfully',
          fields: {'combo': combo, 'action': action.name});
      _lastResult = HotkeyRegistrationResult(
        status: HotkeyRegistrationStatus.success,
      ...
      final isConflict = code == 1409 || pe.code == 'HOTKEY_ALREADY_REGISTERED';
      Log.instance.w('hotkey', 'candidate register failed on OS level',
          fields: {'combo': combo, 'errorCode': code, 'message': pe.message});

      // Rollback previous hotkey if it was active
      if (prevEnabled && prevCombo.isNotEmpty && prevRegistered != null) {
        try {
          await hotKeyManager.register(
            prevRegistered,
            keyDownHandler: (_) => _events.add(HotkeyEvent.keyDown),
      ...
      Log.instance.e('hotkey', 'unexpected candidate register failure', error: e, stack: st);
      if (prevEnabled && prevCombo.isNotEmpty && prevRegistered != null) {
        try {
          await hotKeyManager.register(
            prevRegistered,
