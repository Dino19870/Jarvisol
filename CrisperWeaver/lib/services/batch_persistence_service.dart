...
class BatchPersistenceService {
  ...
  Future<void> saveJob(BatchJob job) {
    return _serial(job.id, () async {
      ...
      final tmp = File(p.join(dir.path, '${_jobFilename(job.id)}.tmp'));
      final encoded =
          const JsonEncoder.withIndent('  ').convert(job.toJson());
      await tmp.writeAsString(encoded, flush: true);
      if (Platform.isWindows && await dst.exists()) {
        try {
          await dst.delete();
        } catch (_) {}
      }
    ...
    await for (final ent in dir.list()) {
      ...
      try {
        ...
        try {
          ...
          Log.instance.w('batch-persist', 'Quarantined corrupt job file to $corruptPath');
        } catch (qErr) {
          Log.instance.e('batch-persist', 'Failed to quarantine corrupt job file: $qErr');
        }
      }
    }
