...
class CrispASREngine implements TranscriptionEngine {
  ...
  Future<bool> loadModel(
    ...
    // Defensive invariant: this engine is the ASR engine.  ModelService's
    // catalogue is shared with TTS/voice/codec entries, so never rely on the
    // UI picker alone to keep an incompatible model out of this path.
    if (lookedUp != null && lookedUp.kind != ModelKind.asr) {
      throw ModelLoadException(
        'Model "$modelId" is ${lookedUp.kind.name}, not an ASR model.',
        engineId,
        modelId,
      );
    }
