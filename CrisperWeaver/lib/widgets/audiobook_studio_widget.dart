...
class _AudiobookStudioWidgetState extends ConsumerState<AudiobookStudioWidget>
  ...
  // Multi-Chapter Selection & Batch Generation State (EXT-V1-04)
  final Set<String> _selectedChapterIdsForBatch = {};
  bool _isBatchRunning = false;
  bool _isBatchCancelled = false;
  int _batchTotalCount = 0;
  int _batchCurrentIndex = 0;
  String? _batchCurrentChapterTitle;
  double _batchCurrentChapterProgress = 0.0;
  final Map<String, AudiobookRenderStatus> _batchChapterStatuses = {};
  final Map<String, String> _batchErrors = {};
  ...
  Map<String, String> _getAvailableVoices([List<ImportedVoicePack>? customPacks]) {
    final settings = ref.read(settingsServiceProvider);
    final map = <String, String>{};

    // 1. Qwen3-TTS Factory Voices
    map['qwen3-uncle_fu'] = '👨 Oncle Fu (Qwen3-TTS — Voix Narrative & Mûre)';
    map['qwen3-ryan'] = '👨 Ryan (Qwen3-TTS — Voix Masculine Dynamique)';
    map['qwen3-eric'] = '👨 Eric (Qwen3-TTS — Voix Masculine Posée)';
    map['qwen3-aiden'] = '👨 Aiden (Qwen3-TTS — Voix Masculine Jeune)';
    map['qwen3-dylan'] = '👨 Dylan (Qwen3-TTS — Voix Masculine Naturelle)';
    map['qwen3-vivian'] = '👩 Vivian (Qwen3-TTS — Voix Féminine Expressive)';
    map['qwen3-serena'] = '👩 Serena (Qwen3-TTS — Voix Féminine Douce)';
    map['qwen3-ono_anna'] = '👩 Anna (Qwen3-TTS — Voix Féminine Chaleureuse)';
    map['qwen3-sohee'] = '👩 Sohee (Qwen3-TTS — Voix Féminine Claire)';

    // 2. Saved Cloned Voices from library
    for (final c in settings.customClonedVoices) {
      map['clone_${c.id}'] = '🎙️ ${c.name} (Clonage Vocal)';
    }

    // 3. Trigger for adding new clone
    map['custom-clone-new'] = '➕ [ + Nouveau clonage vocal (.wav)... ]';

    // 4. Voice Packs importés (.gguf)
    final List<ImportedVoicePack> importedList =
        customPacks ?? ref.read(importedVoiceServiceProvider);
    ...
    // Entrée directe pour importer un Voice Pack depuis le sélecteur
    map['imported-pack-new'] = '📦 [ + Importer un Voice Pack (.gguf)... ]';

    // 5. VibeVoice & Kokoro
    map['vibevoice-voice-fr-Spk0_man'] = '👨 VibeVoice (Homme Français — Voix Naturelle)';
    map['vibevoice-voice-fr-Spk1_woman'] = '👩 VibeVoice (Femme Française — Voix Naturelle)';
    map['kokoro-voice-ff_siwis'] = '👩 Siwis (Kokoro 82M — Voix Rapide)';

    // 6. Rôles assignés dont la voix a été supprimée ou est introuvable
    if (_project != null) {
      for (final spk in _project!.speakers.values) {
        final raw = spk.voiceModelName;
        final canonical = (raw == 'vibevoice-fr-Spk0_man')
            ? 'vibevoice-voice-fr-Spk0_man'
            : (raw == 'vibevoice-fr-Spk1_woman')
                ? 'vibevoice-voice-fr-Spk1_woman'
                : raw;
        if (!map.containsKey(canonical) && !map.containsKey(raw)) {
          map[raw] = '⚠️ [Voix introuvable ou supprimée : $raw]';
        }
      }
    }
  ...
  void initState() {
    super.initState();
    // Charger la liste des Voice Packs importés
    ref.read(importedVoiceServiceProvider.notifier).loadAll().then((_) {
      if (mounted) setState(() {});
    });

    // Load last saved profile
    final settings = ref.read(settingsServiceProvider);
    final savedProfileId = settings.lastAudiobookProfileId;
    final profiles = settings.audiobookRuleProfiles;
  ...
  void dispose() {
    ...
    _player.dispose();
    try {
      Future.value(AppPaths.tmpDir).then((tempDir) {
  ...
  void _initSampleProject() {
    ...
    // Résoudre le profil de clonage pour un voiceModelName donné
    AudiobookSpeaker _resolveClone(AudiobookSpeaker spk) {
      if (!spk.voiceModelName.startsWith('clone_')) return spk;
      if (spk.customVoiceWavPath != null && spk.customVoiceWavPath!.isNotEmpty) return spk;
      final pid = spk.voiceModelName.replaceFirst('clone_', '');
      final matches = settings.customClonedVoices.where((c) => c.id == pid).toList();
      if (matches.isEmpty) return spk;
      final clone = matches.first;
      return spk.copyWith(
        customVoiceWavPath: clone.wavPath,
        customVoiceRefText: (clone.refText != null && clone.refText!.isNotEmpty) ? clone.refText : null,
        speed: clone.defaultSpeed,
        pitch: clone.defaultPitch,
        volume: clone.defaultVolume,
      );
    }

    final defaultSpeakers = <String, AudiobookSpeaker>{
      'narrator': _resolveClone(AudiobookSpeaker(
        id: 'narrator',
        name: 'Narrateur',
        voiceModelName: settings.defaultNarratorVoice,
        role: 'narrator',
      )),
      'male_main': _resolveClone(AudiobookSpeaker(
        id: 'male_main',
        name: 'Personnage Principal (H)',
        voiceModelName: settings.defaultMaleVoice,
        role: 'male',
      )),
      'female_main': _resolveClone(AudiobookSpeaker(
        id: 'female_main',
...
— Reste derrière moi, ordonna John en avançant vers la porte.''',
  ...
  Future<void> _pickEpubFile() async {
    ...
    if (pick.localPaths.isNotEmpty) {
      ...
      final choice = await AudiobookImportDialog.show(
        ...
        profiles: settings.audiobookRuleProfiles,
      );

      if (choice == null) return;
      _activeProfile = choice.profile;
      settings.lastAudiobookProfileId = _activeProfile.id;
      await settings.saveAudiobookRuleProfile(_activeProfile);
  ...
  // ---------------------------------------------------------------------------
  // Gestion de la Distribution des Voix
  // ---------------------------------------------------------------------------

  /// Retire un personnage de la distribution (avec confirmation).
  Future<void> _removeSpeaker(String key, AudiobookSpeaker spk) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: Row(children: [
          ...
          const Text('Retirer le personnage'),
        ]),
        content: Text(
          'Voulez-vous retirer "${spk.name}" de la distribution ?\n\n'
          'Les répliques attribuées à ce personnage ne seront pas supprimées — elles garderont leur label dans le script.',
        ),
        actions: [
  ...
  /// Ouvre l'explorateur Windows dans le dossier du fichier .wav de référence.
  void _openCloneFolder(String wavPath) {
    try {
      final dir = File(wavPath).parent.path;
      Process.run('explorer.exe', [dir]);
    } catch (e) {
      Log.instance.w('audiobook', 'Impossible d\'ouvrir le dossier : $e');
    }
  }

  /// Supprime un profil de voix clonée du registre global (avec confirmation).
  /// Ne supprime PAS le fichier .wav sur le disque.
  Future<void> _deleteClonedVoiceProfile(ClonedVoiceProfile clone) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Row(children: [
          ...
          const SizedBox(width: 8),
          const Text('Supprimer la voix clonée'),
        ]),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Supprimer "${clone.name}" des voix clonées enregistrées ?'),
            ...
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('📁 Fichier WAV de référence :', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 4),
                  Text(clone.wavPath, style: const TextStyle(fontSize: 10, color: Colors.grey)),
                ],
              ),
            ),
            const SizedBox(height: 8),
            const Text(
              '⚠️ Le fichier .wav sur le disque ne sera PAS supprimé.',
        ...
        actions: [
          ...
          FilledButton(
            style: FilledButton.styleFrom(backgroundColor: Colors.red.shade400),
            onPressed: () => Navigator.of(ctx).pop(true),
            child: const Text('Supprimer du registre'),
          ),
        ],
      ),
    );
    ...
    final settings = ref.read(settingsServiceProvider);
    await settings.deleteClonedVoice(clone.id);

    // Si un speaker utilisait cette voix, le repasser sur la voix par défaut
    final updated = Map<String, AudiobookSpeaker>.from(_project!.speakers);
    bool changed = false;
    for (final entry in updated.entries) {
      if (entry.value.voiceModelName == 'clone_${clone.id}') {
        updated[entry.key] = entry.value.copyWith(
          voiceModelName: 'kokoro-voice-am_adam',
          customVoiceWavPath: null,
          customVoiceRefText: null,
        );
        changed = true;
      }
    }
    if (changed) setState(() => _project = _project!.copyWith(speakers: updated));
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('🗑️ Voix clonée "${clone.name}" supprimée du registre.')),
      );
    }
    setState(() {}); // Rafraîchir la liste des voix disponibles
  }

  /// Relocalise le fichier .wav de référence d'un profil de clonage vocal.
  /// Met à jour le profil dans les préférences ET le speaker courant dans le projet.
  Future<void> _relocateClonedVoiceWav(ClonedVoiceProfile clone) async {
    final result = await pickFilesRobust(
      dialogTitle: 'Sélectionner le nouveau fichier .wav pour "${clone.name}"',
      allowedExtensions: ['wav'],
      allowMultiple: false,
    );
    if (result.isEmpty) return;
    final newWavPath = result.localPaths.isNotEmpty ? result.localPaths.first : null;
    if (newWavPath == null || newWavPath.isEmpty) return;

    final settings = ref.read(settingsServiceProvider);

    // 1. Mettre à jour le profil de clonage global dans les préférences
    final updatedClone = clone.copyWith(wavPath: newWavPath);
    settings.saveClonedVoice(updatedClone);

    // 2. Mettre à jour le customVoiceWavPath de tous les speakers qui utilisent ce clone
    final updatedSpeakers = Map<String, AudiobookSpeaker>.from(_project!.speakers);
    bool changed = false;
    for (final entry in updatedSpeakers.entries) {
      if (entry.value.voiceModelName == 'clone_${clone.id}') {
        updatedSpeakers[entry.key] = entry.value.copyWith(customVoiceWavPath: newWavPath);
        settings.saveSpeakerVoiceConfig(entry.key, updatedSpeakers[entry.key]!.toJson());
        changed = true;
      }
    }
    if (changed) {
      ...
      setState(() {}); // Rafraîchir l'affichage du chemin
    }

    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('📎 Voix "${clone.name}" relocalisée : ${File(newWavPath).uri.pathSegments.last}'),
          backgroundColor: Colors.green.shade800,
        ),
      );
    }
  ...
  /// Importe un nouveau fichier .wav pour créer un profil de voix clonée.
  Future<void> _importNewClonedVoice() async {
    final result = await pickFilesRobust(
      dialogTitle: 'Sélectionner le fichier .wav de la voix de référence',
      allowedExtensions: ['wav'],
      allowMultiple: false,
    );
    if (result.isEmpty) return;
    final wavPath = result.localPaths.isNotEmpty ? result.localPaths.first : null;
    if (wavPath == null || wavPath.isEmpty) return;


    // Dialogue pour nommer la voix
    final nameCtrl = TextEditingController(text: File(wavPath).uri.pathSegments.last.replaceAll('.wav', ''));
    final refTextCtrl = TextEditingController(text: 'Bonjour, je suis votre voix de référence en français.');

    final profile = await showDialog<ClonedVoiceProfile>(
      context: context, // ignore: use_build_context_synchronously
      builder: (ctx) => AlertDialog(
        title: const Row(children: [
          Icon(Icons.mic_external_on, color: Colors.pinkAccent),
          SizedBox(width: 8),
          Text('Nouvelle voix clonée'),
        ]),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(
              ...
              decoration: const InputDecoration(
                ...
                hintText: 'Ex: Mélenchon, Narrateur Principal...',
            ...
            Container(
              ...
              child: Row(
                children: [
                  const Icon(Icons.audio_file, size: 14, color: Colors.pinkAccent),
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                labelText: 'Texte de référence (utilisé pour la transcription)',
                hintText: 'Bonjour, je suis votre voix de référence en français.',
                border: OutlineInputBorder(),
                isDense: true,
              ),
              maxLines: 2,
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () {
              ...
              Navigator.of(ctx).pop(ClonedVoiceProfile(
                id: 'clone_${DateTime.now().millisecondsSinceEpoch}',
                name: nameCtrl.text.trim(),
                wavPath: wavPath,
                refText: refTextCtrl.text.trim().isNotEmpty ? refTextCtrl.text.trim() : null,
                createdAt: DateTime.now(),
              ));
            },
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('🎙️ Voix clonée "${profile.name}" importée avec succès !')),
      );
    }
    setState(() {}); // Rafraîchir le dropdown des voix
  }

  /// Importe un nouveau Voice Pack (.gguf) directement depuis Studio Live Audio.
  Future<void> _importVoicePackInStudio() async {
    final result = await pickFilesRobust(
      dialogTitle: 'Sélectionner un Voice Pack (.gguf)',
      allowedExtensions: ['gguf'],
      allowMultiple: false,
    );
    if (result.isEmpty) return;
    final sourcePath = result.localPaths.isNotEmpty ? result.localPaths.first : null;
    if (sourcePath == null || sourcePath.isEmpty) return;

    final info = VoicePackInspector.inspect(sourcePath);
    if (!info.isValid) {
      if (!mounted) return;
      await showDialog<void>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: Text(
            'Le fichier sélectionné ne semble pas être un Voice Pack GGUF valide '
            'ou son format est incompatible (${info.errorMessage ?? 'architecture: "${info.architecture}"'}).',
          ),
          actions: [
    ...
    // Avertissement de transcription anormale (Qwen3 Voice Pack)
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              const Text(
                'Si cette transcription contient du texte qui n\'est pas prononcé dans l\'audio d\'origine, '
                'le modèle vocalisera ce texte en début de synthèse.',
                style: TextStyle(fontStyle: FontStyle.italic, color: Colors.white70, fontSize: 13),
              ),
              const SizedBox(height: 12),
              const Text('Souhaitez-vous quand même importer ce Voice Pack ?'),
            ],
          ),
          actions: [
            TextButton(
    ...
    try {
      final voiceService = ref.read(importedVoiceServiceProvider.notifier);
      final imported = await voiceService.importVoicePack(
        sourcePath,
        overwrite: true,
      );
      if (!mounted) return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('✅ Voice Pack "${imported.displayName}" importé avec succès.'),
  ...
  Future<void> _addNewCustomSpeaker() async {
    ...
    final avail = _getAvailableVoices();
    String selectedVoice = avail.keys.first;
  ...
  Future<bool> _ensureVoiceDownloaded(String voiceModelName) async {
    ...
    // 0. If imported Voice Pack (.gguf)
    if (voiceModelName.startsWith('imported_chatterbox:')) {
      final fileName = voiceModelName.substring('imported_chatterbox:'.length);
      final packFile = File(p.join(AppPaths.importedVoicesDir.path, fileName));
      if (!packFile.existsSync()) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('⚠️ Voice Pack Chatterbox introuvable : "$fileName"'),
      ...
      final baseModel = await modelService.getWhisperCppModelPath('chatterbox-en-q8_0');
      if (baseModel == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('⬇️ Téléchargement du moteur Chatterbox T3 (~640 Mo)...'), duration: Duration(seconds: 5)),
          );
        }
        await modelService.downloadWhisperCppModel('chatterbox-en-q8_0');
      }
      final codecModel = await modelService.getWhisperCppModelPath('chatterbox-s3gen-q8_0');
      if (codecModel == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('⬇️ Téléchargement du codec Chatterbox S3Gen (~365 Mo)...'), duration: Duration(seconds: 5)),
          );
        }
        await modelService.downloadWhisperCppModel('chatterbox-s3gen-q8_0');
      }
      return true;
    }

    if (voiceModelName.startsWith('imported_qwen3:')) {
      final rest = voiceModelName.substring('imported_qwen3:'.length);
      final fileName = rest.contains('#') ? rest.split('#')[0] : rest;
      final packFile = File(p.join(AppPaths.importedVoicesDir.path, fileName));
      if (!packFile.existsSync()) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('⚠️ Voice Pack Qwen3 introuvable : "$fileName"'),
              backgroundColor: Colors.red,
            ),
          );
        }
      ...
      if (baseModel == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('⬇️ Téléchargement du moteur Qwen3 Base (~1.8 Go)...'), duration: Duration(seconds: 5)),
          );
        }
    ...
    // 1. If custom-clone or clone_*, ensure Qwen3 Base + tokenizer are downloaded
    ...
    if (voiceModelName.startsWith('kokoro')) {
      ...
      final voicePath = await modelService.getWhisperCppModelPath(voiceModelName);
      if (voicePath == null && voiceModelName != 'kokoro-82m-q8_0') {
      ...
      final canonicalVoice = (voiceModelName == 'vibevoice-fr-Spk0_man')
          ? 'vibevoice-voice-fr-Spk0_man'
          : (voiceModelName == 'vibevoice-fr-Spk1_woman')
              ? 'vibevoice-voice-fr-Spk1_woman'
              : voiceModelName;
      // 3a. Moteur Realtime 0.5B requis par le catalogue
      final mainModel = await modelService.getWhisperCppModelPath('vibevoice-realtime-0.5b-tts-f16');
      if (mainModel == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('⬇️ Téléchargement du moteur VibeVoice Realtime 0.5B (~1.9 Go)...'), duration: Duration(seconds: 5)),
          );
        }
        await modelService.downloadWhisperCppModel('vibevoice-realtime-0.5b-tts-f16');
      }
      // 3b. Voicepack requis
      final voicePath = await modelService.getWhisperCppModelPath(canonicalVoice);
      if (voicePath == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('⬇️ Téléchargement du voicepack "$canonicalVoice" (~3.5 Mo)...'), duration: const Duration(seconds: 4)),
          );
        }
        await modelService.downloadWhisperCppModel(canonicalVoice);
      }
      return true;
    } else if (voiceModelName.startsWith('vibevoice')) {
      // 3c. Chemin VibeVoice 1.5B préservé pour les fonctions spécifiques
    ...
    try {
      ...
      if (voiceModelName.startsWith('imported_chatterbox:')) {
        ...
        final status = await tts.prepare(
          ...
          voiceName: fileName,
          speakerName: null, // JAMAIS de speaker name sur Chatterbox
        );
        if (!status.ready) {
          throw Exception(status.errorMessage ?? 'Erreur initialisation Chatterbox');
        }
      } else if (voiceModelName.startsWith('imported_qwen3:')) {
        final rest = voiceModelName.substring('imported_qwen3:'.length);
        final fileName = rest.contains('#') ? rest.split('#')[0] : rest;
        final speakerName = rest.contains('#') ? rest.split('#')[1] : null;
        final status = await tts.prepare(
        ...
        if (!status.ready) {
          throw Exception(status.errorMessage ?? 'Erreur initialisation Qwen3');
        }
      } else if (voiceModelName == 'custom-clone' || voiceModelName == 'custom-clone-new' || voiceModelName.startsWith('clone_') || (speaker != null && speaker.customVoiceWavPath != null && speaker.customVoiceWavPath!.isNotEmpty)) {
        final svc = ref.read(audiobookServiceProvider);
        String? wavPath = speaker?.customVoiceWavPath;
        String? refText = speaker?.customVoiceRefText;
        if (wavPath == null && voiceModelName.startsWith('clone_')) {
          final pid = voiceModelName.replaceFirst('clone_', '');
          final settings = ref.read(settingsServiceProvider);
          final match = settings.customClonedVoices.where((c) => c.id == pid).toList();
          if (match.isNotEmpty) {
            wavPath = match.first.wavPath;
            refText = match.first.refText;
          }
        }
        final safeWav = wavPath != null ? await svc.ensure24kHzWav(wavPath) : null;
        await tts.prepare(
          modelName: 'qwen3-tts-12hz-0.6b-base',
          codecName: 'qwen3-tts-tokenizer-12hz',
          voiceWavPath: safeWav,
          refText: refText ?? 'Bonjour, je suis votre voix de référence en français.',
        );
      } else if (voiceModelName.startsWith('vibevoice-voice-') ||
          voiceModelName == 'vibevoice-fr-Spk0_man' ||
          voiceModelName == 'vibevoice-fr-Spk1_woman') {
        final canonicalVoice = (voiceModelName == 'vibevoice-fr-Spk0_man')
            ...
            : (voiceModelName == 'vibevoice-fr-Spk1_woman')
                ...
                : voiceModelName;
        await tts.prepare(
          modelName: 'vibevoice-realtime-0.5b-tts-f16',
  ...
  Future<void> _openVoiceTuning(AudiobookSpeaker speaker) async {
    ...
    final tuned = await VoiceTuningDialog.show(
      ...
      availableVoices: _getAvailableVoices(),
    );
  ...
  void _selectAllChapters() {
    if (_project == null || _isBatchRunning) return;
    setState(() {
      _selectedChapterIdsForBatch.clear();
      _selectedChapterIdsForBatch.addAll(_project!.chapters.map((c) => c.id));
    });
  }
  ...
  Future<String> _renderChapterInternal({
    ...
    for (final spk in _project!.speakers.values) {
      final ok = await _ensureVoiceDownloaded(spk.voiceModelName);
      if (!ok) {
        throw Exception('Modèle vocal non téléchargé pour ${spk.name} : ${spk.voiceModelName}');
      }
    }

    final svc = ref.read(audiobookServiceProvider);
    final appDocsDir = AppPaths.dataDir;
  ...
  Future<void> _synthesizeCurrentChapter() async {
    if (_project == null || _isSynthesizing || _isBatchRunning) return;
    ...
    try {
      final outPath = await _renderChapterInternal(
        chapter: chapter,
        onProgress: (p) {
  ...
  Future<void> _startBatchChapterGeneration() async {
    if (_project == null || _isBatchRunning || _isSynthesizing) return;

    // 1. Snapshot queue ordered strictly by book order (chapter.index)
    final queue = _project!.chapters
        .where((c) => _selectedChapterIdsForBatch.contains(c.id))
        .toList()
      ..sort((a, b) => a.index.compareTo(b.index));

    if (queue.isEmpty) return;

    // 2. Pre-flight check for already existing audio files or ready status
    final appDocsDir = AppPaths.dataDir;
    final outDir = '${appDocsDir.path}/Audiobooks/${_project!.title}';
    final alreadyExisting = queue.where((c) {
      final outPath = p.join(outDir, 'Audiobook_Chap_${c.index.toString().padLeft(2, '0')}.wav');
      return c.status == AudiobookRenderStatus.ready || File(outPath).existsSync();
    }).toList();

    List<AudiobookChapter> chaptersToRun = List.from(queue);
    int skippedCount = 0;

    if (alreadyExisting.isNotEmpty && mounted) {
        ...
        builder: (ctx) => AlertDialog(
          title: const Row(
            children: [
              ...
              SizedBox(width: 8),
              Text('Chapitres déjà générés'),
            ],
          ),
          content: Text(
            '${alreadyExisting.length} des ${queue.length} chapitres sélectionnés ont déjà un fichier audio généré.\n\n'
            'Que souhaitez-vous faire pour ces chapitres ?',
          ),
          actions: [
            TextButton(
            ...
            OutlinedButton(
              ...
              child: const Text('Régénérer et remplacer'),
            ),
            FilledButton(
              onPressed: () => Navigator.pop(ctx, 'skip'),
              child: const Text('Ignorer ceux déjà générés'),
            ),
          ],
        ),
      );
      ...
      if (result == 'skip') {
        final existingIds = alreadyExisting.map((c) => c.id).toSet();
        chaptersToRun = queue.where((c) => !existingIds.contains(c.id)).toList();
        skippedCount = alreadyExisting.length;
        if (chaptersToRun.isEmpty) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Tous les chapitres sélectionnés sont déjà générés.'),
                backgroundColor: Colors.green,
              ),
            );
          }
    ...
    // 3. Verify all required character voices are available
    for (final spk in _project!.speakers.values) {
      final ok = await _ensureVoiceDownloaded(spk.voiceModelName);
      if (!ok) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Synthèse par lot annulée : voix non prête pour ${spk.name} (${spk.voiceModelName})'),
              backgroundColor: Colors.red,
            ),
          );
        }
    ...
    // 4. Initialize batch execution state
    setState(() {
      _isBatchRunning = true;
      _isBatchCancelled = false;
      _batchTotalCount = chaptersToRun.length;
      _batchCurrentIndex = 0;
      _batchCurrentChapterProgress = 0.0;
      _batchErrors.clear();
      for (final c in chaptersToRun) {
        _batchChapterStatuses[c.id] = AudiobookRenderStatus.waiting;
      }
    });
    ...
    // 5. Sequential loop: 1 chapter at a time in book order
    for (int i = 0; i < chaptersToRun.length; i++) {
      if (_isBatchCancelled) {
        for (int k = i; k < chaptersToRun.length; k++) {
          _batchChapterStatuses[chaptersToRun[k].id] = AudiobookRenderStatus.cancelled;
        }
        break;
      }

      final currentChap = chaptersToRun[i];

      setState(() {
        _batchCurrentIndex = i + 1;
        _batchCurrentChapterTitle = 'Chapitre ${currentChap.index} : ${currentChap.title}';
        _batchCurrentChapterProgress = 0.0;
        _batchChapterStatuses[currentChap.id] = AudiobookRenderStatus.synthesizing;
      });
      ...
      try {
        final outPath = await _renderChapterInternal(
          ...
          onProgress: (p) {
            if (mounted) {
              setState(() => _batchCurrentChapterProgress = p);
            }
          },
        );

        final updatedChapter = currentChap.copyWith(
          status: AudiobookRenderStatus.ready,
          audioFilePath: outPath,
          progress: 1.0,
          errorMessage: null,
        ...
        final updatedList = List<AudiobookChapter>.from(_project!.chapters);
        final chIdx = updatedList.indexWhere((c) => c.id == currentChap.id);
        if (chIdx != -1) {
          updatedList[chIdx] = updatedChapter;
        }

        if (mounted) {
          setState(() {
            _project = _project!.copyWith(chapters: updatedList);
            _batchChapterStatuses[currentChap.id] = AudiobookRenderStatus.ready;
          });
        }
        successCount++;
      } catch (e) {
        final errStr = e.toString();
        _batchErrors[currentChap.id] = errStr;

        final updatedChapter = currentChap.copyWith(
          status: AudiobookRenderStatus.error,
          errorMessage: errStr,
        );
        ...
        if (mounted) {
          setState(() {
            _project = _project!.copyWith(chapters: updatedList);
            _batchChapterStatuses[currentChap.id] = AudiobookRenderStatus.error;
          });
        }
        failCount++;
    ...
    if (mounted) {
      ...
      _showBatchSummaryDialog(
        total: chaptersToRun.length,
        succeeded: successCount,
        failed: failCount,
        skipped: skippedCount,
        wasCancelled: _isBatchCancelled,
      );
    }
  }

  void _stopBatchChapterGeneration() {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('⏹️ Arrêt du lot demandé... Fin du chapitre en cours.'),
      ...
      builder: (ctx) {
        ...
        return AlertDialog(
          title: Row(
            children: [
              Icon(
                failed == 0 && !wasCancelled
                    ? Icons.check_circle
                    : (wasCancelled ? Icons.stop_circle : Icons.warning),
                color: failed == 0 && !wasCancelled
                    ? Colors.green
                    : (wasCancelled ? Colors.orange : Colors.red),
              ),
              const SizedBox(width: 8),
              const Text('Bilan de la génération par lot'),
            ],
          ),
          content: ConstrainedBox(
            ...
            child: Column(
              ...
              children: [
                Text(
                  wasCancelled
                      ? '⚠️ La génération par lot a été interrompue.'
                      : (failed == 0
                          ? '🎉 Génération par lot terminée avec succès !'
                          : '⚠️ Génération par lot terminée avec des erreurs.'),
                  style: const TextStyle(fontWeight: FontWeight.bold),
                ),
                const SizedBox(height: 12),
                Text('• Total chapitres traités : $total'),
                Text('• Chapitres générés avec succès : $succeeded',
                    style: const TextStyle(color: Colors.green)),
                if (skipped > 0)
                  Text('• Chapitres ignorés (déjà prêts) : $skipped',
                      style: const TextStyle(color: Colors.blueAccent)),
                if (failed > 0)
                  Text('• Chapitres en échec : $failed',
                      style: const TextStyle(color: Colors.red)),
                if (wasCancelled)
                  const Text('• Arrêté par l\'utilisateur',
                ...
                if (_batchErrors.isNotEmpty) ...[
                  ...
                  Container(
                    ...
                    child: ListView(
                      shrinkWrap: true,
                      children: _batchErrors.entries.map((e) {
                        final chap = _project?.chapters.firstWhere(
                          (c) => c.id == e.key,
                          orElse: () => AudiobookChapter(
                              id: e.key, index: 0, title: e.key, rawText: ''),
                        );
                        return Padding(
                          padding: const EdgeInsets.only(bottom: 4.0),
                          child: Text(
                            'Chapitre ${chap?.index ?? '?'}: ${e.value}',
            ...
            if (failed > 0)
              OutlinedButton.icon(
                ...
                onPressed: () {
                  ...
                  setState(() {
                    _selectedChapterIdsForBatch.clear();
                    _selectedChapterIdsForBatch.addAll(_batchErrors.keys);
                  });
                  _startBatchChapterGeneration();
                },
              ),
            FilledButton(
  ...
  Widget build(BuildContext context) {
    super.build(context);
    // Watch reactive imported voice packs so that any import/delete anywhere in the app
    // immediately rebuilds the Studio with the updated voice list.
    final importedPacks = ref.watch(importedVoiceServiceProvider);
    Log.instance.i('Studio', 'E_R5_STUDIO_REACTIVE_PROVIDER_ACTIVE');
    Log.instance.i('Studio', 'E_R5_IMPORTED_PACK_COUNT=${importedPacks.length}');
    for (final pack in importedPacks) {
      Log.instance.i('Studio', 'E_R5_PACK: id=${pack.id}, fileName=${pack.fileName}, family=${pack.family.name}, voices=${pack.voiceNames}, exists=${pack.fileExists}');
    }
                        ...
                        child: Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              // Chapters Tab Header with Selection Actions
                              Container(
                                  ...
                                  children: [
                                    Row(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                          '${_selectedChapterIdsForBatch.length}/${_project!.chapters.length} sélectionné(s)',
                                          style: theme.textTheme.labelSmall?.copyWith(
                                            color: cs.primary,
                                    ...
                                    Wrap(
                                      ...
                                      children: [
                                        TextButton.icon(
                                          ...
                                          label: const Text('Sélectionner tous les chapitres'),
                                          onPressed: _isBatchRunning ? null : _selectAllChapters,
                                        ),
                                        TextButton.icon(
                                          style: TextButton.styleFrom(
                                            visualDensity: VisualDensity.compact,
                                          ...
                                          label: const Text('Désélectionner tous les chapitres'),
                                          onPressed: _isBatchRunning ? null : _deselectAllChapters,
                                        ),
                                      ],
                                    ),
                                  ],
                              ...
                              Expanded(
                                ...
                                child: ListView.builder(
                                  ...
                                  itemBuilder: (ctx, i) {
                                    ...
                                    final isBatchChecked = _selectedChapterIdsForBatch.contains(chap.id);
                                    final batchStatus = _batchChapterStatuses[chap.id];

                                    // Resolve display status
                                    Widget statusTrailing;
                                    if (_isBatchRunning && batchStatus == AudiobookRenderStatus.synthesizing) {
                                      statusTrailing = const Tooltip(
                                    ...
                                    } else if (_isBatchRunning && batchStatus == AudiobookRenderStatus.waiting) {
                                      statusTrailing = const Tooltip(
                                        message: 'En attente dans le lot (○)',
                                        child: Icon(Icons.radio_button_unchecked, size: 16, color: Colors.grey),
                                      );
                                    } else if (batchStatus == AudiobookRenderStatus.cancelled) {
                                      statusTrailing = const Tooltip(
                                        message: 'Lot interrompu (■)',
                                        child: Icon(Icons.stop_circle_outlined, size: 16, color: Colors.orange),
                                      );
                                    } else if (chap.status == AudiobookRenderStatus.ready) {
                                      statusTrailing = const Tooltip(
                                        message: 'Prêt (✓)',
                                        child: Icon(Icons.check_circle, size: 16, color: Colors.green),
                                      );
                                    } else if (chap.status == AudiobookRenderStatus.error || batchStatus == AudiobookRenderStatus.error) {
                                      statusTrailing = Tooltip(
                                        message: chap.errorMessage ?? _batchErrors[chap.id] ?? 'Erreur (⚠)',
                                    ...
                                    return ListTile(
                                      ...
                                      leading: Row(
                                        ...
                                        children: [
                                          Checkbox(
                                            value: isBatchChecked,
                                            onChanged: _isBatchRunning
                                                ? null
                                                : (val) {
                                                    setState(() {
                                                      if (val == true) {
                                                        _selectedChapterIdsForBatch.add(chap.id);
                                                      } else {
                                                        _selectedChapterIdsForBatch.remove(chap.id);
                                                      }
                                                    });
                                                  },
                                            visualDensity: VisualDensity.compact,
                                          ...
                                          CircleAvatar(
                                            ...
                                            child: Text(
                                              ...
                                              style: TextStyle(
                                                fontSize: 9,
                                                color: chap.status == AudiobookRenderStatus.ready
                                                    ? Colors.white
                                                    : cs.onSurface,
                                              ),
                                            ),
                              ...
                              // Batch Action & Progress Box at bottom of TOC
                              if (_isBatchRunning)
                                Container(
                                    ...
                                    children: [
                                      Row(
                                        ...
                                        children: [
                                          Expanded(
                                            child: Text(
                                              'Génération des chapitres : $_batchCurrentIndex / $_batchTotalCount',
                                          ...
                                          OutlinedButton.icon(
                                            ...
                                            label: const Text('Arrêter le lot'),
                                            onPressed: _stopBatchChapterGeneration,
                                          ),
                                        ],
                                      ),
                                      if (_batchCurrentChapterTitle != null) ...[
                                        const SizedBox(height: 4),
                                        Text(
                                          _batchCurrentChapterTitle!,
                                          maxLines: 1,
                                          overflow: TextOverflow.ellipsis,
                                      ...
                                      LinearProgressIndicator(
                                        value: _batchTotalCount > 0
                                            ? ((_batchCurrentIndex - 1).clamp(0, _batchTotalCount) + _batchCurrentChapterProgress) / _batchTotalCount
                                            : null,
                                      ),
                                      const SizedBox(height: 4),
                                      Text(
                                        'Chapitre en cours : ${(_batchCurrentChapterProgress * 100).toInt()}%',
                                        style: const TextStyle(fontSize: 10, color: Colors.grey),
                                      ),
                                    ],
                                  ),
                                ...
                                Padding(
                                  ...
                                  child: SizedBox(
                                    ...
                                    child: FilledButton.icon(
                                      ...
                                      onPressed: _selectedChapterIdsForBatch.isEmpty
                                          ? null
                                          : _startBatchChapterGeneration,
                                      icon: const Icon(Icons.auto_mode, size: 16),
                                      label: Text(
                                        'Générer les chapitres sélectionnés (${_selectedChapterIdsForBatch.length})',
                              ...
                              Container(
                                ...
                                child: Row(
                                  children: [
                                    ...
                                    // Bouton : Importer une nouvelle voix clonée
                                    IconButton(
                                      icon: const Icon(Icons.mic_external_on, size: 16),
                                      tooltip: 'Importer une voix clonée (.wav)',
                                      visualDensity: VisualDensity.compact,
                                      onPressed: _importNewClonedVoice,
                                    ),
                                    // Bouton : Importer un Voice Pack (.gguf)
                                    IconButton(
                                      icon: const Icon(Icons.inventory_2_outlined, size: 16),
                                      tooltip: 'Importer un Voice Pack (.gguf)',
                                      visualDensity: VisualDensity.compact,
                                      onPressed: _importVoicePackInStudio,
                                    ),
                              ...
                              Expanded(
                                ...
                                child: ListView(
                                  ...
                                  children: _project!.speakers.entries.map((e) {
                                    final spk = e.value;
                                    // Détecter si voix clonée active
                                    final isClone = spk.voiceModelName.startsWith('clone_');
                                    ClonedVoiceProfile? activeClone;
                                    if (isClone) {
                                      final pid = spk.voiceModelName.replaceFirst('clone_', '');
                                      final matches = ref.read(settingsServiceProvider).customClonedVoices.where((c) => c.id == pid);
                                      if (matches.isNotEmpty) activeClone = matches.first;
                                    }

                                    // Détecter si Voice Pack importé manquant
                                    bool isMissingImported = false;
                                    String? missingPackName;
                                    if (spk.voiceModelName.startsWith('imported_')) {
                                      String fn;
                                      if (spk.voiceModelName.startsWith('imported_chatterbox:')) {
                                        fn = spk.voiceModelName.substring('imported_chatterbox:'.length);
                                      } else if (spk.voiceModelName.startsWith('imported_qwen3:')) {
                                        final rest = spk.voiceModelName.substring('imported_qwen3:'.length);
                                        fn = rest.contains('#') ? rest.split('#')[0] : rest;
                                      } else {
                                        fn = spk.voiceModelName.replaceFirst('imported_', '');
                                      }
                                      final pFile = File(p.join(AppPaths.importedVoicesDir.path, fn));
                                      if (!pFile.existsSync()) {
                                        isMissingImported = true;
                                        missingPackName = fn;
                                      }
                                    }
                                          ...
                                          children: [
                                            if (isMissingImported) ...[
                                              Container(
                                                ...
                                                child: Row(
                                                  children: [
                                                    Icon(Icons.warning_amber_rounded, size: 14, color: Colors.amber.shade300),
                                                    const SizedBox(width: 6),
                                                    Expanded(
                                                      child: Text(
                                                        'Voice Pack "$missingPackName" manquant sur le disque. Veuillez réassigner.',
                                                        style: TextStyle(color: Colors.amber.shade200, fontSize: 10),
                                                      ),
                                                    ),
                                                  ],
                                                ),
                                            ...
                                            Row(
                                              children: [
                                                ...
                                                // Bouton Supprimer (sauf pour le Narrateur)
                                                if (spk.id != 'narrator')
                                                  IconButton(
                                                    icon: Icon(Icons.person_remove, size: 14, color: Colors.red.shade300),
                                                    tooltip: 'Retirer "${spk.name}" de la distribution',
                                                    visualDensity: VisualDensity.compact,
                                                    onPressed: () => _removeSpeaker(e.key, spk),
                                                  ),
                                              ],
                                            ),
                                             Builder(
                                               builder: (ctx) {
                                                 final avail = _getAvailableVoices(importedPacks);
                                                 return DropdownButtonFormField<String>(
                                                   key: ValueKey('speaker_${spk.id}_${avail.length}_${spk.voiceModelName}'),
                                                   isDense: true,
                                                   isExpanded: true,
                                                   initialValue: avail.containsKey(spk.voiceModelName)
                                                       ? spk.voiceModelName
                                                       : avail.keys.first,
                                                   items: avail.entries.map((voice) {
                                                   ...
                                                   onChanged: (newVoice) {
                                                     if (newVoice != null) {
                                                       if (newVoice == 'imported-pack-new') {
                                                         _importVoicePackInStudio();
                                                         return;
                                                       }
                                                       if (newVoice == 'custom-clone-new' || (newVoice == 'custom-clone' && (spk.customVoiceWavPath == null || spk.customVoiceWavPath!.isEmpty))) {
                                                         _openVoiceTuning(spk);
                                                         return;
                                                       }
                                                      final settings = ref.read(settingsServiceProvider);
                                                      AudiobookSpeaker updatedSpk = spk.copyWith(voiceModelName: newVoice);
                                                      if (newVoice.startsWith('clone_')) {
                                                        final pid = newVoice.replaceFirst('clone_', '');
                                                        final match = settings.customClonedVoices.where((c) => c.id == pid).toList();
                                                        if (match.isNotEmpty) {
                                                          final clone = match.first;
                                                          updatedSpk = updatedSpk.copyWith(
                                                            customVoiceWavPath: clone.wavPath,
                                                            customVoiceRefText: clone.refText,
                                                            speed: clone.defaultSpeed,
                                                            pitch: clone.defaultPitch,
                                                            volume: clone.defaultVolume,
                                                          );
                                                        }
                                                      } else if (!newVoice.startsWith('custom-clone')) {
                                                        updatedSpk = updatedSpk.copyWith(
                                                          customVoiceWavPath: null,
                                                          customVoiceRefText: null,
                                                        );
                                            ...
                                            // Badge voix clonée : chemin du WAV + actions
                                            if (activeClone != null) ...[
                                              const SizedBox(height: 4),
                                              Builder(builder: (ctx) {
                                                final wavExists = File(activeClone!.wavPath).existsSync();
                                                return Row(
                                                  children: [
                                                    Icon(
                                                      wavExists ? Icons.mic : Icons.mic_off,
                                                      size: 11,
                                                      color: wavExists ? Colors.pinkAccent : Colors.red.shade400,
                                                    ),
                                                    const SizedBox(width: 4),
                                                    Expanded(
                                                      child: Tooltip(
                                                    ...
                                                    // ✏️ Relocaliser le fichier .wav
                                                    InkWell(
                                                      onTap: () => _relocateClonedVoiceWav(activeClone!),
                                                      child: Tooltip(
                                                        message: 'Relocaliser le fichier .wav',
                                                        child: Padding(
                                                    ...
                                                    // 📁 Ouvrir le dossier contenant le .wav
                                                    InkWell(
                                                      onTap: () => _openCloneFolder(activeClone!.wavPath),
                                                      child: Tooltip(
                                                        message: 'Ouvrir le dossier du fichier .wav',
                                                        child: Padding(
                                                          padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 2),
                                                          child: Icon(Icons.folder_open, size: 13, color: Colors.amber.shade400),
                                                        ),
                                                      ),
                                                    ),
                                                    // 🗑️ Supprimer la voix clonée du profil global
                                                    InkWell(
                                                      onTap: () => _deleteClonedVoiceProfile(activeClone!),
                                                      child: Tooltip(
                                                        message: 'Supprimer "${activeClone!.name}" des voix clonées',
                                                        child: Padding(
                                                          padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 2),
                                                          child: Icon(Icons.delete_forever, size: 13, color: Colors.red.shade300),
                                                        ),
                                                      ),
                                                    ),
                                                  ],
                                          ...
                                          child: Wrap(
                                            ...
                                            children: [
                                              ...
                                              if (_selectedLineIds.isNotEmpty)
                                                FilledButton.tonalIcon(
                                                  onPressed: (_isSynthesizing || _isBatchRunning) ? null : _synthesizeSelectedBatch,
                                              ...
                                              if (_isSynthesizing)
                                                ...
                                                FilledButton.icon(
                                                  onPressed: _isBatchRunning ? null : _synthesizeCurrentChapter,
