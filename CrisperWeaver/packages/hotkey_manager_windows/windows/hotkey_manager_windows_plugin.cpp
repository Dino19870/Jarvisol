...
void HotkeyManagerWindowsPlugin::Register(
  ...
  HWND target_hwnd = ::GetAncestor(registrar_->GetView()->GetNativeWindow(), GA_ROOT);
  ::SetLastError(0);
  BOOL reg_ok = ::RegisterHotKey(
      target_hwnd,
      hotkey_id, fs_modifiers, key_code);
  if (!reg_ok) {
    DWORD err = ::GetLastError();
    --last_registered_hotkey_id_;
    std::string err_code_str = (err == 1409) ? "HOTKEY_ALREADY_REGISTERED" : "REGISTER_HOTKEY_FAILED";
    std::ostringstream err_msg;
    err_msg << "RegisterHotKey failed with error " << err;
    flutter::EncodableMap err_details;
    err_details[flutter::EncodableValue("errorCode")] = static_cast<int64_t>(err);
    err_details[flutter::EncodableValue("identifier")] = identifier;
    result->Error(err_code_str, err_msg.str(), flutter::EncodableValue(err_details));
    return;
  }
