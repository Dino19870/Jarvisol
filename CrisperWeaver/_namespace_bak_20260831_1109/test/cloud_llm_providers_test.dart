...
import 'package:crisper_weaver/models/cloud_llm_provider_profile.dart';
import 'package:crisper_weaver/services/settings_service.dart';
import 'package:crisper_weaver/services/llm_service.dart';
import 'package:crisper_weaver/widgets/llm_settings_dialog.dart';
...
void main() {
  ...
  group('AUDIT CLOUD LLM PROVIDERS : Modèles & Persistance', () {
    test('1.1 CloudLlmProviderProfile serialization, presets, and copyWith', () {
      ...
      final copied = profile.copyWith(name: 'Mistral Production', defaultModel: 'pixtral-large-latest');
      ...
      expect(CloudLlmProviderProfile.presets.any((p) => p.name.contains('OpenAI')), isTrue);
      expect(CloudLlmProviderProfile.presets.any((p) => p.name.contains('Groq')), isTrue);
      expect(CloudLlmProviderProfile.presets.any((p) => p.name.contains('DeepSeek')), isTrue);
    });

    test('1.2 SettingsService save, list, and delete custom cloud providers', () async {
      ...
      // Update p1
      final updatedP1 = p1.copyWith(name: 'OpenAI Prod', defaultModel: 'gpt-4o-mini');
  ...
  group('AUDIT CLOUD LLM PROVIDERS : Dialogue & Actions Utilisateur', () {
    ...
    test('2.1 saveCloudProvider persists active profile on first add', () async {
      final profile = CloudLlmProviderProfile(
      ...
      expect(settingsService.customCloudProviders.first.cachedModels, ['gpt-4o', 'gpt-4o-mini']);
    });

    test('2.2 switching to local provider clears activeCloudProviderId', () async {
    ...
    test('2.3 CloudLlmProviderProfile presets cover all major providers', () {
      ...
      expect(presets.any((p) => p.endpoint.contains('openai.com')), isTrue, reason: 'OpenAI preset manquant');
      expect(presets.any((p) => p.endpoint.contains('groq.com')), isTrue, reason: 'Groq preset manquant');
      expect(presets.any((p) => p.endpoint.contains('mistral.ai')), isTrue, reason: 'Mistral preset manquant');
      expect(presets.any((p) => p.endpoint.contains('deepseek.com')), isTrue, reason: 'DeepSeek preset manquant');
      expect(presets.any((p) => p.endpoint.contains('openrouter.ai')), isTrue, reason: 'OpenRouter preset manquant');

      // All presets have a non-empty defaultModel
      for (final preset in presets) {
        expect(preset.defaultModel.isNotEmpty, isTrue, reason: 'Preset "${preset.name}" sans defaultModel');
        expect(preset.cachedModels.isNotEmpty, isTrue, reason: 'Preset "${preset.name}" sans cachedModels');
      }
    });

    test('2.4 Manual model ID entry — profile saves with user-supplied model string', () async {
      ...
      final saved = settingsService.customCloudProviders.firstWhere((p) => p.id == 'prov_manual');
