...
void main() {
  ...
  group('TESTS DE COHÉRENCE DIALOGUE IA & MULTIMODAL (Images & Documents)', () {
    test('1. Traitement Multimodal d une Image avec OCR et extraction contextuelle', () async {
...
''';
      ...
      final imageItem = DocumentSourceItem(
        ...
        path: fakeImagePath,
        type: 'image',
        textContent: '[Contenu textuel extrait par OCR de l\'image "Scan_Facture_PC_SOFT.png"] :\n---\n$extractedOcrText\n---',
        sizeBytes: 154200,
      ...
      const userQuery = 'Quel est le montant total TTC et jusqu à quand est-il valable ?';
      ...
      expect(foundText.contains('1 788,00 EUR') || foundText.contains('Total TTC'), isTrue);
      expect(foundText.contains('30/06/2026'), isTrue);
    });

    test('2. Cohérence Sémantique de la Réponse IA sur document complexe', () async {
...
''';
      ...
      expect(foundText.contains('tempe gauche') || foundText.contains('contondant'), isTrue);
      ...
      expect(foundText2.contains('Aucune trace') || foundText2.contains('arme à feu'), isTrue);
    });

    test('3. Dialogue Multi-Tours : Conservation du contexte et non-hallucination', () {
      final messages = [
        LlmChatMessage(role: 'user', content: 'Qui est l enquêteur principal ?', timestamp: DateTime.now()),
        LlmChatMessage(role: 'assistant', content: 'L enquêteur principal est le capitaine Pete Marino.', timestamp: DateTime.now()),
        LlmChatMessage(role: 'user', content: 'Et qui pratique l autopsie ?', timestamp: DateTime.now()),
        LlmChatMessage(role: 'assistant', content: 'L autopsie est pratiquée par le docteur Kay Scarpetta.', timestamp: DateTime.now()),
      ];

      expect(messages.length, equals(4));
      final promptMessages = messages.map((m) => '${m.role}: ${m.content}').join('\n');
