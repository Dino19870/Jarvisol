...
template <typename T>
// NOTE: CrispASR carried a batch-1 occupancy variant of this kernel (gated by
// CRISPASR_METAL_IM2COL_OCC) that blocked OW across thread-dim0, worth ~2x on
// melotts HiFi-GAN. It was DROPPED in the v0.17 sync: upstream restructured the
// im2col dispatch in ggml-metal-ops.cpp, so the optimisation no longer applies to
// the code it was measured against. Re-derive and re-benchmark against the new
// dispatch before reintroducing — do not paste the old kernel back.
