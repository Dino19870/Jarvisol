...
CA_EXPORT int crispasr_session_set_voice(crispasr_session* s, const char* path, const char* ref_text_or_null) {
    ...
    if (s->voice_is_clone && !s->logged_clone_consent) {
        ...
        if (tm_info) {
            std::strftime(ts, sizeof(ts), "%Y-%m-%dT%H:%M:%S", tm_info);
        } else {
            snprintf(ts, sizeof(ts), "1970-01-01T00:00:00");
        }
#endif
...
#ifdef CA_HAVE_QWEN3_TTS
    if (s->qwen3_tts_ctx) {
        if (qwen3_tts_is_custom_voice(s->qwen3_tts_ctx)) {
            int rc = qwen3_tts_set_speaker_by_name(s->qwen3_tts_ctx, name);
            if (rc == 0)
                s->qwen3_tts_voice_loaded = true;
            return rc;
        }
        int rc = qwen3_tts_select_voice(s->qwen3_tts_ctx, name);
        if (rc == 0)
            s->qwen3_tts_voice_loaded = true;
        return rc;
    }
...
CA_EXPORT int crispasr_session_n_speakers(crispasr_session* s) {
    ...
    if (s->qwen3_tts_ctx) {
        ...
        return qwen3_tts_voice_pack_n_voices(s->qwen3_tts_ctx);
    }
#endif
#ifdef CA_HAVE_MELOTTS
    if (s->melotts_ctx)
...
CA_EXPORT const char* crispasr_session_get_speaker_name(crispasr_session* s, int i) {
    ...
    if (s->qwen3_tts_ctx) {
        ...
        return qwen3_tts_voice_pack_get_voice_name(s->qwen3_tts_ctx, i);
    }
#endif
    return nullptr;
}
...
static void crispasr_session_warn_unmarked_clone(crispasr_session* s) {
    ...
    std::time_t t_warn = std::time(nullptr);
    struct tm tm_buf_warn;
    char ts[64] = {0};
#ifdef _WIN32
    if (localtime_s(&tm_buf_warn, &t_warn) == 0) {
        std::strftime(ts, sizeof(ts), "%Y-%m-%dT%H:%M:%S", &tm_buf_warn);
    } else {
        snprintf(ts, sizeof(ts), "1970-01-01T00:00:00");
    }
#else
    struct tm* tm_info_warn = std::localtime(&t_warn);
    if (tm_info_warn) {
        std::strftime(ts, sizeof(ts), "%Y-%m-%dT%H:%M:%S", tm_info_warn);
    } else {
        snprintf(ts, sizeof(ts), "1970-01-01T00:00:00");
    }
#endif
