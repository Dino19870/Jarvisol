...
struct qwen3_tts_context {
    ...
    std::vector<float> cached_tts_pad_emb;  // cached tts_pad embedding for AR trailing steps (Python parity)
...
bool build_icl_prefill_embeds(qwen3_tts_context* c, const std::string& syn_text, const std::string& ref_text,
    ...
    c->cached_tts_pad_emb.assign(tts_pad, tts_pad + d);
    ...
    // Strict parity with official Qwen3-TTS (modeling_qwen3_tts.py lines 1978-1981):
    // ref_content is passed as-is without destructive token clamps or heuristics.
    // Quality, duplication, and alignment guards are enforced at the authoring source (VoiceBake).
...
extern "C" int qwen3_tts_voice_pack_n_voices(struct qwen3_tts_context* ctx) {
    if (!ctx) return 0;
    return (int)ctx->vp_names.size();
}

extern "C" const char* qwen3_tts_voice_pack_get_voice_name(struct qwen3_tts_context* ctx, int idx) {
    if (!ctx || idx < 0 || (size_t)idx >= ctx->vp_names.size()) return nullptr;
    return ctx->vp_names[idx].c_str();
}
...
static bool qwen3_tts_generate_codes_ar(qwen3_tts_context* ctx, const char* text, std::vector<int32_t>& all_codes,
    ...
    for (frame = 0; frame < max_frames; frame++) {
        ...
        // Add trailing_text_hidden[gen_step] (or tts_pad if past M, per Python modeling_qwen3_tts.py l. 1689-1692).
        const float* trail = (frame < M_trail)
                                 ? (trailing.data() + (size_t)frame * d)
                                 : (!ctx->cached_tts_pad_emb.empty()
                                        ? ctx->cached_tts_pad_emb.data()
                                        : trailing.data() + (size_t)(M_trail - 1) * d);
        for (int j = 0; j < d; j++) {
            next_emb[j] += trail[j];
        }
        if (bench) {
