...
bool phonemize_cached(kokoro_context* ctx, const std::string& lang, const std::string& text, std::string& out) {
    ...
    std::string eff_lang = lang;
    if (eff_lang.empty() || eff_lang == "auto") {
        if (ctx && !ctx->vp.name.empty()) {
            const std::string& v = ctx->vp.name;
            if (v.rfind("af_", 0) == 0 || v.rfind("am_", 0) == 0 || v.rfind("bf_", 0) == 0 || v.rfind("bm_", 0) == 0) {
                eff_lang = "en";
            } else if (v.rfind("ff_", 0) == 0 || v.rfind("fm_", 0) == 0) {
                eff_lang = "fr";
            } else if (v.rfind("df_", 0) == 0 || v.rfind("dm_", 0) == 0) {
                eff_lang = "de";
            } else if (v.rfind("ef_", 0) == 0 || v.rfind("em_", 0) == 0) {
                eff_lang = "es";
            }
        }
        if (eff_lang.empty() || eff_lang == "auto") {
    ...
    auto try_builtin = [&]() -> bool {
        if (skip_builtin)
            return false;
        bool ok = false;
        if (eff_lang == "en" || eff_lang == "en-us" || eff_lang == "en-gb") {
            ...
            ok = crispasr::phonemize_misaki_en(eff_lang, text, out);
            if (!ok)
                ok = crispasr::phonemize_builtin_en(eff_lang, text, out, /*misaki_style=*/true);
        } else if (eff_lang == "de")
            ok = crispasr::phonemize_builtin_de(eff_lang, text, out, kokoro_punctuation());
        else if (eff_lang == "fr" || eff_lang == "fr-fr")
            ok = crispasr::phonemize_builtin_fr(eff_lang, text, out, kokoro_punctuation());
        else if (eff_lang == "es" || eff_lang == "es-es")
            ok = crispasr::phonemize_builtin_es(eff_lang, text, out, kokoro_punctuation());
    ...
    auto try_espeak = [&]() -> bool {
        if (skip_espeak)
            return false;
#if defined(CRISPASR_HAVE_ESPEAK_NG) || defined(CRISPASR_ESPEAK_DLOPEN)
        if (phonemize_espeak_lib(eff_lang, effective_text, out)) {
            crispasr::strip_espeak_lang_markers(out); // #169
            return true;
        }
#endif
        if (phonemize_popen(eff_lang, effective_text, out)) {
            crispasr::strip_espeak_lang_markers(out); // #169
            return true;
        }
        return false;
