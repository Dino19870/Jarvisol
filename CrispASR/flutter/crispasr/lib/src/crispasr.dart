...
TextLanguage? detectTextLanguage(
  ...
  try {
    ...
    if (rc != 0) return null;
    final label = labelBuf.cast<Utf8>().safeToDartString();
...
RegistryBundle? registryDefaultBundle(String backend, {DynamicLibrary? lib}) {
  ...
  try {
    ...
    for (var index = 0; index < count; index++) {
      ...
      try {
        ...
        artifacts.add(RegistryArtifact(
          ...
          filename: filenameBuf.cast<Utf8>().safeToDartString(),
          url: urlBuf.cast<Utf8>().safeToDartString(),
          approxSize: sizeBuf.cast<Utf8>().safeToDartString(),
        ));
      } finally {
        calloc.free(kindPtr);
    ...
    return RegistryBundle(
      backend: canonicalBuf.cast<Utf8>().safeToDartString(),
      license: licenseBuf.cast<Utf8>().safeToDartString(),
...
RegistryEntry? _registryCall(String sym, String key, DynamicLibrary? lib) {
  ...
  if (rc == 0) {
    out = RegistryEntry(
      filename: fnBuf.cast<Utf8>().safeToDartString(),
      url: urlBuf.cast<Utf8>().safeToDartString(),
      approxSize: sizeBuf.cast<Utf8>().safeToDartString(),
    );
  }
...
String? cacheEnsureFile(
  ...
  final path = rc == 0 ? outBuf.cast<Utf8>().safeToDartString() : null;
  calloc.free(fnPtr);
  calloc.free(urlPtr);
  calloc.free(ovPtr);
  calloc.free(outBuf);
...
String? cacheDir({String? override, DynamicLibrary? lib}) {
  ...
  final dir = rc == 0 ? outBuf.cast<Utf8>().safeToDartString() : null;
  calloc.free(ovPtr);
  calloc.free(outBuf);
  return dir;
}
...
List<AlignedWord> alignWords({
  ...
  for (var i = 0; i < n; i++) {
    ...
    final t = tp == nullptr ? '' : tp.safeToDartString();
    out.add(AlignedWord(
      text: t,
...
LidResult detectLanguagePcm({
  ...
  double conf = -1;
  if (rc == 0) {
    code = outBuf.cast<Utf8>().safeToDartString();
    conf = outConf.value;
  }
...
class CrispASR {
  ...
  List<Segment> _collectSegments({required bool wantWords}) {
    ...
    for (var i = 0; i < n; i++) {
      ...
      final text = textPtr == nullptr ? '' : textPtr.safeToDartString();
      ...
      if (wantWords &&
        ...
        for (var k = 0; k < nTokens; k++) {
          ...
          final tok = tp == nullptr ? '' : tp.safeToDartString();
          ...
          if (_tokenNAlts != null &&
            ...
            if (nA > 0) {
              ...
              for (var a = 0; a < nA; a++) {
                ...
                final atext = ts == nullptr ? '' : ts.safeToDartString();
  ...
  LanguageDetection detectLanguage(
    ...
    try {
      ...
      final code = p >= 0 ? outCode.safeToDartString() : '';
...
class CrispasrSession {
  ...
  factory CrispasrSession.open(
    ...
    try {
      ...
      final be = bp == nullptr ? '' : bp.safeToDartString();
  ...
  static List<String> _availableBackends(DynamicLibrary lib) {
    ...
    try {
      ...
      fn(ptr, cap);
      final csv = ptr.safeToDartString();
      return csv.isEmpty
          ? const <String>[]
  ...
  String detectedLanguage() {
    ...
    try {
      ...
      final s = buf.cast<Utf8>().safeToDartString();
  ...
  List<SessionSegment> _readSegments(Pointer<Void> res) {
    ...
    for (var i = 0; i < nSegs; i++) {
      ...
      final text = tp == nullptr ? '' : tp.safeToDartString();
      ...
      final spk = spkP == nullptr ? '' : spkP.safeToDartString();
      final wc = nWords(res, i);
      final words = <Word>[];
      for (var k = 0; k < wc; k++) {
        final wp = wordText(res, i, k);
        final wt = wp == nullptr ? '' : wp.safeToDartString();
        ...
        if (wordNAltsFn != null &&
          ...
          if (nA > 0) {
            ...
            for (var a = 0; a < nA; a++) {
              ...
              final atext = tp2 == nullptr ? '' : tp2.safeToDartString();
              if (atext.isEmpty) continue;
  ...
  List<String>? ctcVocab() {
    ...
    for (var i = 0; i < n; i++) {
      ...
      vocab[i] = p == nullptr ? '' : p.safeToDartString();
    }
    return vocab;
  }
  ...
  String? translateText(String text, String srcLang, String tgtLang,
    ...
    try {
      ...
      final out = res.safeToDartString();
  ...
  ({String lang, double confidence}) detectLanguage(
    ...
    try {
      ...
      return (lang: outBuf.cast<Utf8>().safeToDartString(), confidence: probPtr[0]);
    } finally {
      calloc.free(pcmPtr);
      calloc.free(pathPtr);
      calloc.free(outBuf);
  ...
  List<String> speakers() {
    ...
    for (var i = 0; i < n; i++) {
      ...
      if (ptr != nullptr) out.add(ptr.safeToDartString());
    }
    return out;
  }
    ...
    try {
      ...
      if (pcmPtr == nullptr || n <= 0) {
        ...
        if (_lib.providesSymbol('crispasr_session_last_synth_error')) {
          ...
          if (errPtr != nullptr) {
            final msg = errPtr.safeToDartString();
  ...
  ({Float32List pcm, String transcript}) speechToSpeech(Float32List inputPcm) {
    ...
    try {
      ...
      if (txtPtr != nullptr) {
        transcript = txtPtr.safeToDartString();
        calloc.free(txtPtr);
      }
      try {
  ...
  List<Stem> separate(Float32List pcmStereo) {
    ...
    try {
      ...
      for (var i = 0; i < nStems; i++) {
        ...
        final name = namePtr == nullptr ? 'stem$i' : namePtr.safeToDartString();
...
class PuncModel {
  ...
  String process(String text) {
    ...
    final result = resultPtr.safeToDartString();
    freeFn(resultPtr);
    return result;
  }
...
class CrispasrSpeakerDB {
  ...
  (String?, double) match(Float32List embedding, {double threshold = 0.7}) {
    ...
    String? name;
    if (score >= threshold) {
      name = nameBuf.cast<Utf8>().safeToDartString();
    }
    malloc.free(nameBuf);
    return (name, score);
  }
...
class CrispasrSpeakerEmbedder {
  ...
  String get name {
    ...
    return p == nullptr ? '' : p.safeToDartString();
  }
...
String? detectBackendFromGguf(String path, {String? libPath}) {
  ...
  try {
    ...
    if (rc <= 0) return null;
    final name = outBuf.cast<Utf8>().safeToDartString();
...
String? kokoroResolveModelForLang(String modelPath, String lang,
  ...
  try {
    ...
    if (rc != 0) return null;
    return outBuf.cast<Utf8>().safeToDartString();
  } finally {
    calloc.free(mp);
    calloc.free(lp);
    calloc.free(outBuf);
...
({String voicePath, String voiceName})? kokoroResolveFallbackVoice(
  ...
  try {
    ...
    if (rc != 0) return null;
    return (
      voicePath: pathBuf.cast<Utf8>().safeToDartString(),
      voiceName: nameBuf.cast<Utf8>().safeToDartString(),
    );
  } finally {
    calloc.free(mp);
    calloc.free(lp);
    ...
    for (var i = 0; result.length < count; i++) {
      if (bytes[i] == 0) {
        result.add(Pointer<Utf8>.fromAddress(ptr.address + start)
            .safeToDartString(length: i - start));
        start = i + 1;
      }
    }
    return result;
...
extension _SafePointerUtf8 on Pointer<Utf8> {
  String safeToDartString({int? length}) {
    ...
    try {
      ...
      try {
        ...
        final bytes = cast<Uint8>().asTypedList(len);
