        ...
        if (!gctx) {
                ...
                if (ref_t && ref_t->type == GGML_TYPE_F32) {
                    ...
                    for (int t : { 0, 1, 10, 50, 100, 143 }) {
                        ...
                        printf("  token %3d: cos=%.6f max_abs=%.4e  cpp=[%.4f %.4f %.4f] ref=[%.4f %.4f %.4f] %s\n", t,
                               tc, max_abs, enc[t * hidden], enc[t * hidden + 1], enc[t * hidden + 2],
            ...
            if (logits_ref && logits_ref->type == GGML_TYPE_F32) {
                ...
                if (gen_ids) {
                    ...
                    for (int i = 0; i < std::min(n_ids, 20); i++) printf("%d ", ids[i]);
                    printf("\n");
                }
                ...
                for (int i = 0; i < 5; i++) printf("%d(%.3f) ", scored[i].second, scored[i].first);
                printf("\n");
            }
    ...
    if (argc >= 3) {
        ...
        if (ext && strcmp(ext, ".bin") == 0) {
            ...
            if (bf) {
                ...
                extern const char * ppformulanet_ocr_recognize_chw(ppformulanet_ocr_context *, const float *, int *);
                int olen = 0;
