...
std::string tokenizer::decode(const int32_t * ids, int n) const {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (piece.size() > 3 && piece[0] == '<' && piece[1] == '|' && piece.back() == '>' &&
            piece[piece.size() - 2] == '|')
            continue; // <|...|>
...
void tokenizer::build_reverse_map() {
    piece_to_id.clear();
    for (int i = 0; i < (int)id_to_piece.size(); i++) piece_to_id[id_to_piece[i]] = i;
    // Find special tokens
...
vision_graph build_vision_graph(context & ctx) {
    ...
    size_t meta_size =
        (size_t)(n_layers * 40 + 100) * ggml_tensor_overhead() + ggml_graph_overhead_custom(16384, false);
...
void pixel_unshuffle_v2(const float * in, float * out, int n_patches_per_side, int hidden_size,
...
proj_graph build_proj_graph(context & ctx, int n_merged) {
    ...
    size_t meta_size = 32 * ggml_tensor_overhead() + ggml_graph_overhead_custom(256, false);
...
llm_graph build_llm_graph(context & ctx, int n_tokens, int n_past, bool use_kv_cache = false) {
    llm_graph lg;
    const auto & lhp = ctx.m.lhp;
    ...
    for (int i = 0; i < n_layers; i++) {
        ...
        // Apply RoPE
        Q = ggml_rope_ext(g, Q, rope_pos, nullptr, hd, GGML_ROPE_TYPE_NEOX, 0, lhp.rope_theta, 1.0f, 0.0f, 1.0f, 0.0f,
                          0.0f);
        K_new = ggml_rope_ext(g, K_new, rope_pos, nullptr, hd, GGML_ROPE_TYPE_NEOX, 0, lhp.rope_theta, 1.0f, 0.0f, 1.0f,
                              0.0f, 0.0f);
        ...
        float scale = 1.0f / std::sqrt((float)hd);
        ggml_tensor * attn_out = ggml_flash_attn_ext(g, Q, Kfull, Vfull, mask, scale, 0.0f, 0.0f);
        attn_out = ggml_reshape_2d(g, attn_out, D, T);
...
void free_(context & ctx) {
    free_kv_cache(ctx);
    if (ctx.vis_graph_ctx) {
        ggml_free(ctx.vis_graph_ctx);
...
bool encode_vision_tile(context & ctx, const float * pixels, vision_result & out) {
    ...
    ggml_backend_tensor_set(vg.pixel_in, patches.data(), 0, n_patches * patch_flat * sizeof(float));
    ...
    ggml_backend_tensor_get(vg.output, out.hidden, 0, n_pos * D * sizeof(float));
...
bool project_vision(context & ctx, const float * vis_hidden, int n_patches, project_result & out) {
    ...
    ggml_backend_tensor_set(pg.input, unshuffled.data(), 0, n_merged * merge_dim * sizeof(float));
    ...
    ggml_backend_tensor_get(pg.output, out.embeds, 0, n_merged * out_dim * sizeof(float));

    ggml_free(pg.gctx);
    return true;
}

bool encode_vision(context & ctx, const float * tiles, int n_tiles, vision_pipeline_result & out) {
    ...
    for (int t = 0; t < n_tiles; t++) {
        ...
        std::memcpy(all_embeds + t * n_merged_per_tile * out_dim, pr.embeds,
                    n_merged_per_tile * out_dim * sizeof(float));
    ...
    out.embed_dim = out_dim;
    if (bench) {
        auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - t_vis_total)
                      .count();
...
bool run_llm_forward(context & ctx, const int32_t * token_ids, int n_tokens, llm_result & out,
    ...
    if (mask) {
        ggml_backend_tensor_set(mask, mask_data.data(), 0, T * T * sizeof(ggml_fp16_t));
    }
    ...
    if (lg.logits_out) {
        ...
        ggml_backend_tensor_get(lg.logits_out, out.logits, 0, T * V * sizeof(float));
    }
...
static bool run_cached_step(context & ctx, const int32_t * token_ids, int n_tokens, int n_past,
    ...
    for (int qi = 0; qi < T; qi++) {
        ...
        for (int ki = 0; ki < Lk; ki++) {
            mask_data[(size_t)qi * Lk + ki] = ggml_fp32_to_fp16((ki > q_pos) ? -INFINITY : 0.0f);
        }
    }
    ggml_tensor * mask = ggml_graph_get_tensor(lg.gf, "causal_mask");
    if (mask) {
        ggml_backend_tensor_set(mask, mask_data.data(), 0, (size_t)Lk * T * sizeof(ggml_fp16_t));
    }
    ...
    if (splice && n_past == 0) {
        ...
        if (splice->token_to_image) {
            for (int t = 0; t < T; t++) {
                ...
                if (img_idx >= 0 && img_idx < splice->n_image_tokens) {
                    ...
                    for (int d = 0; d < D; d++) {
                        img_data[(size_t)t * D + d] = splice->image_embeds[(size_t)img_idx * D + d];
        ...
        if (img_t) {
            ggml_backend_tensor_set(img_t, img_data.data(), 0, (size_t)D * T * sizeof(float));
        }
        if (mask_t) {
            ggml_backend_tensor_set(mask_t, mask_f.data(), 0, (size_t)D * T * sizeof(float));
        }
    } else if (n_past == 0) {
        ...
        if (img_t) {
            std::vector<float> zeros((size_t)D * T, 0.0f);
            ggml_backend_tensor_set(img_t, zeros.data(), 0, (size_t)D * T * sizeof(float));
        }
        if (mask_t) {
            std::vector<float> ones((size_t)D * T, 1.0f);
            ggml_backend_tensor_set(mask_t, ones.data(), 0, (size_t)D * T * sizeof(float));
        }
    }
...
const char * internvl2_ocr_recognize_raw(internvl2_ocr_context * ctx, const uint8_t * pixel_bytes, int width,
    ...
    std::vector<int32_t> prompt_tokens = ctx->ctx.tok.build_prompt(ctx->prompt, n_img_tokens);

    // Generate
    internvl2_ocr::generate_result gen;
    bool ok = internvl2_ocr::generate(ctx->ctx, vpr.image_embeds, vpr.n_image_tokens, vpr.embed_dim,
                                      prompt_tokens.data(), (int)prompt_tokens.size(), ctx->max_tokens, gen);
