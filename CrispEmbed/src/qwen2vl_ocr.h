...
struct vision_hparams {
    ...
    std::vector<int> deepstack_indexes;                         // Qwen3-VL: multi-layer feature concat
                                                                // (e.g. [5,11,17])
...
struct context {
    ...
    // Persistent decode graph — built once per generate() call, reused every
    // step.
    struct decode_cache {
...
bool run_llm_forward(context & ctx, const int32_t * token_ids, int n_tokens, llm_result & out,
                     const image_input * img = nullptr, bool logits_last_only = false, bool materialize_hidden = false,
