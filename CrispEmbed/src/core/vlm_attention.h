...
struct RoPEFreqTable {
    ...
    void precompute(int hd, float theta) {
        ...
        int half = hd / 2;
        freqs.resize(half);
        for (int d = 0; d < half; d++) freqs[d] = 1.0f / powf(theta, 2.0f * d / hd);
    }
...
static inline void apply_rope(float * qk, int n_heads, int head_dim, int position, float theta, RoPEStyle style) {
    int half = head_dim / 2;
...
static inline std::vector<float> alloc_kv_cache(int n_layers, int max_seq, int n_kv_heads, int head_dim) {
...
static inline size_t kv_k_offset(int layer, int seq_pos, int kv_dim, int max_seq, int n_layers) {
    (void)n_layers;
    return (size_t)layer * 2 * max_seq * kv_dim + (size_t)seq_pos * kv_dim;
}

// Offset to V[layer][seq_pos] in the flat cache.
static inline size_t kv_v_offset(int layer, int seq_pos, int kv_dim, int max_seq, int n_layers) {
    (void)n_layers;
    return (size_t)layer * 2 * max_seq * kv_dim + (size_t)max_seq * kv_dim + (size_t)seq_pos * kv_dim;
}
...
static inline void gqa_attn_step(const float * q, const float * k_new, const float * v_new, float * kv_cache,
                                 int n_heads, int n_kv_heads, int head_dim, int max_seq, int n_past, int layer_idx,
                                 int n_layers, float * output, float attn_scale = -1.0f) {
    ...
    memcpy(kv_cache + k_base + (size_t)n_past * kv_dim, k_new, kv_dim * sizeof(float));
    memcpy(kv_cache + v_base + (size_t)n_past * kv_dim, v_new, kv_dim * sizeof(float));
    ...
    for (int h = 0; h < n_heads; h++) {
        ...
        for (int k = 0; k < Lk; k++) {
            float s =
                core_cpu::dot_product(q + h * head_dim, kv_cache + k_base + k * kv_dim + kv_h * head_dim, head_dim) *
                scale;
            scores[k] = s;
            if (s > max_s) max_s = s;
        }
        ...
        for (int d = 0; d < head_dim; d++) {
            float val = 0;
            for (int k = 0; k < Lk; k++) val += scores[k] * inv * kv_cache[v_base + k * kv_dim + kv_h * head_dim + d];
...
static inline void swiglu_ffn(const float * x, float * out, int hidden_dim, int intermediate_dim, const float * gate_w,
    ...
    core_cpu::linear_cpu(x, tl_gate.data(), hidden_dim, intermediate_dim, gate_w, nullptr);
    core_cpu::linear_cpu(x, tl_up.data(), hidden_dim, intermediate_dim, up_w, nullptr);

    // silu(gate) * up
    for (int i = 0; i < intermediate_dim; i++) tl_gate[i] = core_cpu::silu(tl_gate[i]) * tl_up[i];

    core_cpu::linear_cpu(tl_gate.data(), out, intermediate_dim, hidden_dim, down_w, nullptr);
}
