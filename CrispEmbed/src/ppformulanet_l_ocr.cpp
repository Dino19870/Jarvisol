...
static void map_tensors(ppformulanet_l_ocr_context * ctx) {
    ...
    for (int i = 0; i < hp.enc_layers; i++) {
        ...
        l.ln1_w = EL("ln1.weight");
        l.ln1_b = EL("ln1.bias");
        l.qkv_w = EL("attn.qkv.weight");
        l.qkv_b = EL("attn.qkv.bias");
        l.proj_w = EL("attn.proj.weight");
        l.proj_b = EL("attn.proj.bias");
        l.rel_pos_h = EL("attn.rel_pos_h");
        l.rel_pos_w = EL("attn.rel_pos_w");
        l.ln2_w = EL("ln2.weight");
        l.ln2_b = EL("ln2.bias");
        l.mlp_lin1_w = EL("mlp.lin1.weight");
        l.mlp_lin1_b = EL("mlp.lin1.bias");
        l.mlp_lin2_w = EL("mlp.lin2.weight");
        l.mlp_lin2_b = EL("mlp.lin2.bias");
    }

    // Neck
    ctx->neck_conv1_w = F(m, "enc.neck.conv1.weight");
    ctx->neck_conv2_w = F(m, "enc.neck.conv2.weight");
    ctx->neck_ln1_w = F(m, "enc.neck.ln1.weight");
    ctx->neck_ln1_b = F(m, "enc.neck.ln1.bias");
    ctx->neck_ln2_w = F(m, "enc.neck.ln2.weight");
    ctx->neck_ln2_b = F(m, "enc.neck.ln2.bias");
...
static ggml_cgraph * build_full_layer_graph(ggml_context * g, ppformulanet_l_ocr_context * ctx, int li, int C, int T,
                                            int aH, int aW, int nW, int n_heads, bool skip_ln1 = false) {
...
static void run_encoder_graph(ppformulanet_l_ocr_context * ctx, const float * rgb_chw, int imgH, int imgW) {
    const auto & hp = ctx->hparams;
    ...
    for (int py = 0; py < nP; py++) {
        for (int px = 0; px < nP; px++) {
            ...
            for (int o = 0; o < C; o++) {
                float s = pe_b[o];
                for (int i = 0; i < patch_dim; i++) s += pe_w[o * patch_dim + i] * patch[i];
    ...
    for (int li = 0; li < hp.enc_layers; li++) {
        ...
        ggml_cgraph * gf = build_full_layer_graph(gc, ctx, li, C, T, aH, aW, nW, n_heads, skip_ln1);
    ...
    for (int y = 0; y < nP; y++)
        for (int x = 0; x < nP; x++)
            for (int c = 0; c < C; c++) nchw[c * nP * nP + y * nP + x] = hidden[(y * nP + x) * C + c];

    auto nc1_w = to_f32(ctx->neck_conv1_w);
    std::vector<float> neck1(nC * nP * nP);
    conv2d_cpu(nchw.data(), neck1.data(), nc1_w.data(), nullptr, C, nC, nP, nP, 1, 1, 1, 0, 1);
    ...
    conv2d_cpu(neck1.data(), neck2.data(), nc2_w.data(), nullptr, nC, nC, nP, nP, 3, 3, 1, 1, 1);
    ...
    conv2d_cpu(neck2.data(), pc1.data(), pc1_w.data(), nullptr, nC, mid_ch, nP, nP, 3, 3, 2, 1, 1);
    ...
    for (int y = 0; y < h2; y++)
        for (int x = 0; x < w2; x++)
            for (int c = 0; c < out_ch; c++) flat[(y * w2 + x) * out_ch + c] = pc2[c * h2 * w2 + y * w2 + x];
    ...
    for (int n = 0; n < n_tokens; n++) {
        ...
        linear_cpu(flat.data() + n * out_ch, l1.data(), out_ch, out_ch, pl1_w.data(), pl1_b.data());
        linear_cpu(l1.data(), ctx->proj_out.data() + n * dec_dim, out_ch, dec_dim, pl2_w.data(), pl2_b.data());
    }

    fprintf(stderr, "ppfnl: projector done, shape=(%d, %d)\n", n_tokens, dec_dim);
}

// (Scalar SAM-ViT encoder removed — was ~370 lines of dead code, never called.
//  The ggml graph encoder (run_encoder_graph) is used exclusively.)
// ---------------------------------------------------------------------------
...
static void precompute_cross_kv(ppformulanet_l_ocr_context * ctx) {
    ...
    for (int li = 0; li < hp.dec_layers; li++) {
        ...
        for (int t = 0; t < n_enc; t++) {
            linear_cpu(ctx->proj_out.data() + t * D, ctx->cross_k_cache[li].data() + t * D, D, D, kw.data(), kb.data());
            linear_cpu(ctx->proj_out.data() + t * D, ctx->cross_v_cache[li].data() + t * D, D, D, vw.data(), vb.data());
        }
    }
}
...
static std::vector<float> decoder_step(ppformulanet_l_ocr_context * ctx, int token, int step,
    ...
    for (int i = 0; i < D; i++) x[i] = tok_data[token * D + i] * scale + pos_data[(step + 2) * D + i];
...
const char * ppformulanet_l_ocr_recognize(ppformulanet_l_ocr_context * ctx, const float * pixels, int width, int height,
    ...
    if (bench)
        fprintf(stderr, "[ppfn_l-bench] preprocess: %.1f ms\n",
                std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - t0).count());

    t0 = std::chrono::steady_clock::now();
    run_encoder_graph(ctx, rgb.data(), S, S);
    if (bench)
        fprintf(stderr, "[ppfn_l-bench] encoder: %.1f ms\n",
                std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - t0).count());
    ...
    if (bench)
        fprintf(stderr, "[ppfn_l-bench] decoder: %.1f ms\n",
                std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - t0).count());

    if (bench)
        fprintf(stderr, "[ppfn_l-bench] total: %.1f ms\n",
...
const char * ppformulanet_l_ocr_recognize_beam(ppformulanet_l_ocr_context * ctx, const float * pixels, int width,
    ...
    if (!ctx || !pixels) return nullptr;
    if (beam_width <= 1) return ppformulanet_l_ocr_recognize(ctx, pixels, width, height, out_len);
